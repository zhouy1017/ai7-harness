...
This checkout carries the provider-free AI7 new-Book tracer, the complete J-02 long-manuscript editing module, the cohesive J-08 interrupted-manuscript recovery module, and the bounded J-12 Book/Revision Workbench and Product Data Location slice. A local DOCX is reviewed before the initial Book graph is created atomically; the resulting Manuscript can then be edited through bounded windows, outline and whole-manuscript navigation, CJK search, frozen atomic replacement, Milestone Versions, durable undo/redo, persisted restart/reopen, recovery-first comparison/restoration, and exact immutable historical-Revision viewing. Distinct exact Books may coexist in non-focus-stealing Workbench windows while each Book has at most one. This remains intentionally narrower than full J-01, the whole Book Workspace, or canonical J-12.
...
J-12 creates one clearly public synthetic DOCX and a second empty Book only inside its disposable external root. It exercises the production renderer/main/private-stdio service/Harness/store path to prove distinct exact-Book Workbench coexistence, serialized duplicate reuse, background no-focus-steal behavior, current offline editing, a later Revision followed by exact immutable earlier-Revision reconstruction capped at 32 blocks, return to current editable state, sender-owned native picker provenance, clean restart persistence, and truthful `设置 > 数据与存储` platform/runtime/root/bounded-footprint/main-owned-reveal/Protected-Secret-Store meaning. Applicable keyboard/IME/focus/200%-reflow/forced-colors behavior and the zero-request loopback sentinel are included. The controller reads no database oracle, records no manuscript payload/screenshot/trace/video/artifact, and deletes the exact disposable runtime state at cleanup. It is the admitted bounded J-12 slice, not the credential/Provider branches owned by Issue #46 or the full canonical journey.
