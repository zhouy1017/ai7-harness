...
J-12 creates one clearly public synthetic DOCX, a second empty Book, and runtime-only synthetic credential values inside its disposable external root/controller. It exercises the production renderer/main/private-stdio service/Harness/store path to prove distinct exact-Book Workbench coexistence, serialized duplicate reuse, background no-focus-steal behavior, current offline editing, exact immutable Revision history, restart persistence, and truthful `设置 > 数据与存储`. Its role-first `设置 > 模型服务` path proves four role cards, the one exact Main Editorial DeepSeek binding, real OS-protected save without redisplay, restart, same-reference replacement, removal, removal restart, schema-v11 nonsecret persistence, data-root byte absence, fixed development-ci Provider Processing v1 zero-transmission policy, separate External Export v1, no Public Release Permission, and zero configured Harness providers/agents/sessions. Applicable keyboard/IME/focus/200%-reflow/forced-colors behavior and the zero-request loopback sentinel are included. A `finally` path removes any synthetic OS-store entry before deleting runtime state. The controller reads no semantic database oracle, records no credential/manuscript payload/screenshot/trace/video/artifact, and never makes a Provider call.
