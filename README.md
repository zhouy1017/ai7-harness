...
J-12 creates one clearly public synthetic DOCX, a second empty Book, and runtime-only synthetic credential values inside its disposable external root/controller. It exercises the production renderer/main/private-stdio service/Harness/store path to prove distinct exact-Book Workbench coexistence, serialized duplicate reuse, background no-focus-steal behavior, current offline editing, exact immutable Revision history, restart persistence, and truthful `设置 > 数据与存储`. Its role-first `设置 > 模型服务` path proves four role cards, the one exact Main Editorial DeepSeek binding, real OS-protected save without redisplay, restart, same-reference replacement, removal, removal restart, exact canonical strict schema-v11 nonsecret persistence, data-root absence of raw secrets and common SHA-256-derived encodings, visibly verified development-ci Provider Processing v1 zero-transmission policy, separate External Export v1, no Public Release Permission, and zero configured Harness providers/agents/sessions. It also verifies the exact OS-store value after first save and replacement and its absence after product removal without outputting those values. Applicable keyboard traversal/focus visibility, IME/focus, 200%-reflow, forced-colors behavior, and the zero-request loopback sentinel are included. Cancellation and ordinary exit share one idempotent `finally` cleanup promise: product removal is preferred, direct OS-store removal succeeds only when the native adapter confirms deletion, and an unacquired reference may be recovered only from one exact nonsecret metadata row in the synthetic Agent Data Root after the product is closed. That cleanup-only read is not a scenario oracle. No credential/manuscript payload, screenshot, trace, video, or artifact is recorded, and no Provider call is made.
