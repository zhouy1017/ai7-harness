...
Sample manuscripts are governed separately and more narrowly. They are real manuscripts, authorized for AI7 use **only while kept private and local**: never committed in history or working tree, never in hosted CI, build artifacts, distributable fixtures, corpora, or the shipped product, and carrying no redistribution right. Sending their text to any external model service is a separate decision this authorization does not grant.
