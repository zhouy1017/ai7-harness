...
Consume Harness as exactly pinned public npm packages — no fork, no vendored source — and take only the subset AI7's composition needs. Never depend on the `@deepseek-ai/dsh` CLI aggregate: it transitively installs the generic shell, pwsh, terminal, and web tool packages that the editorial tool surface excludes, and absence from the dependency graph is a stronger guarantee than absence from the wiring. Pin exact versions with a committed lockfile and one coherent version across the selected subset; never use `^`, `~`, or `latest`, because `latest` still points at `0.0.1-rc.1` on nearly every package while `next` is `0.1.0-rc.6`. The consumed baseline is `0.1.0-rc.6`; `0.1.0-rc.5` is the audited but uninstallable reference and the two must never be conflated. Upstream publishes no tags and no releases, so AI7 tracks commits and npm versions itself. Pin bumps enter one at a time through a dedicated pull request verifying the composed Cordis config diff, capability exposure diff, session schema compatibility, AI7 journey replay, regenerated third-party notices, and Node/Electron ABI; compilation is never acceptance.
...
Verification uses two GitHub Actions workflows, `pr` and `release`, each a single job on `windows-2025`. Windows is the only target platform and the only place required evidence is produced. `pr` is the sole required gate and carries no path filters. `release` builds one Windows package on a `v*` tag and fails closed without a green `pr` run for the same source SHA. Focused verification is local and has no workflow; Provider Rehearsal is local, opt-in, and never gating. Required CI remains provider-free: no live model call, no API key, no unpublished manuscript text. Keep the request-fingerprint guard so replay fails closed when the effective request drifts from its recorded fixture, regenerate the legacy public-synthetic corpus rather than byte-copying it, and emit a five-field release receipt.
