...
A Ubuntu lane, a nightly tier, a machine-owned Test Catalog, a quarantine registry, and the wire-level fault server are deferred, each behind a trigger recorded in `kick-in/26-tiered-verification-and-mock-provider-evidence.md`. Add verification machinery when a concrete problem appears, not in advance. Preserve local multi-agent dispatch as a repository-development workflow only, never as shipped AI7 runtime behavior.
