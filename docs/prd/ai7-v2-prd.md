...
44. As a professional editor, I want 快速开始 to run under a Default Execution Rule that I created from a viewed plan with 设为快速开始默认 and that matches only a newly user-initiated Task, so that speed never schedules ambient background work or grants Apply authority (ADR 0077).
...
88. As an editorial workflow owner, I want to save an exact Milestone Version of the Manuscript and designate one as the 发稿版本, so that important internal states are durable and 发稿 is explicit without implying publication (ADR 0077).
...
90. As an editorial workflow owner, I want each Production Document handed over through a Delivery Record and one destination- and format-independent 图书交付包 after 发稿 and every delivery, so that 发稿, 交付 and the package stay distinct and no identity collapses into one exported file (ADR 0077).
...
94. As an editorial authority, I want to designate the Manuscript's exact 发稿版本 separately from any document's 交付, so that publication status and delivery status remain distinct (ADR 0077).
...
- The 图书交付包's identity is destination- and format-independent and Book-level; a Production Document is delivered through a Delivery Record; Publication Version and Public Release Permission belong to the Manuscript and remain separate from package creation and local export (ADR 0077).
- External Export Policy v2 is default-deny and makes only an exact user-selected local-file Effect eligible, over the five target kinds of ADR 0079 §3; v1 remains its immutable predecessor. Each file still requires frozen preparation, exact approval, atomic commit, verification, and its own receipt or classified outcome.
- Trusted build/launch authority maps development/CI to immutable Provider Processing v1, exact `sample1` fixture recording to immutable v2, ordinary production to default-deny v6, and the developer-live scope of ADR 0065 to v5 through active-policy-set v5 (ADR 0079 §2); v3 and v4 remain their immutable predecessors. This policy selection is current design authority but proves no executable selector, adapter, endpoint, model binding, credential, dispatch path, or live call exists.
...
- [ADR 0076](../adr/0076-align-editor-facing-surfaces-to-the-owner-s-september-decisions.md) and [ADR 0077](../adr/0077-adopt-the-editor-facing-surface-specification-as-the-execution-standard.md) (2026-09-09) specify every editor-facing surface in [`docs/ui-ux-v2/editor-surfaces.md`](../ui-ux-v2/editor-surfaces.md); stories 44, 88, 90 and 94 and the delivery decision above were re-read under them on 2026-09-10 ([ADR 0078](../adr/0078-align-the-remaining-references-the-plan-and-the-tracker-to-the-editor-facing-specification.md)). Capabilities the specification adds without a story here — editorial marks, categorized 审阅, 评估 and 审稿意见, the seven Knowledge Base classes with a local Material Index, Book People, the Data Version with database export, import and backup, Production Documents and writing tasks — are requirements by those ADRs; the story list gains them in a later PRD revision the Owner authorizes.
