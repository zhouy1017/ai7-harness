...
v7 reproduces v5's decision exactly except on its one eligible-only rule, where `webSearchToolAllowed` is `true`, a `platformTools` block names AI7's two platform tools exactly as ADR 0080 §7.5 writes them (`websearch`: service `parallel`, host `search.parallel.ai`, tool `web_search`, anonymous; `webfetch`: 5,242,880 bytes, 30 seconds, bounded by citations), and the per-frozen-unit Run Budget Ceiling default is 90,000 tokens rather than 30,000, because a search-enabled unit spends model turns on tool results and the real cost is unknown until the first search-enabled Run (ADR 0080 §7.4). Its `authorityBasis` gains ADR 0080. The Owner reviewed these bytes on 2026-09-12 and settled both proposals: the search service is Parallel as the Commander proposed, and the per-frozen-unit default is 90,000 rather than the 60,000 the Commander proposed. Provider Processing v5 stays selected and becomes v7's immutable predecessor record.
