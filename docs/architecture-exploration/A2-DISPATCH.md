...
Status: **A2 review-clean at `f1d212c5`; U2 accepted; stable `0.149.0` App Server x64 package selected as X2; corrected static retry completed as `probe partial` and awaits evidence review; all downloaded-binary execution, candidate re-score, and A3 remain blocked**
...
From authorization head `01774b54490bc723879ed165c49d5f48f7cc53d5`, the Commander performed that one fresh attempt. [The corrected-retry evidence](./A2-STATIC-ARTIFACT-PROBE-RETRY-EVIDENCE.md) reports **probe partial**: the exact package identity matched; both archive passes safely matched all nine records and captured a six-file inventory; the App Server entrypoint digest matched; cache-only, no-UI, non-revocation trust observations and packaged-file absence were recorded; cleanup passed; both trees remained exact and clean; and no downloaded binary ran. Review found one malformed 65-character `codex-package.json` hash field. A one-character-deletion candidate reconciles the raw aggregate exactly but is inference only because the original report object/serialized bytes are gone; that file digest and the aggregate fingerprint remain unusable. Exact-head re-review is required. The evidence does not authorize candidate re-score, dynamic execution, A3, DeepSeek re-entry, maintenance selection, or implementation.
