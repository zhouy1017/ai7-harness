...
| `AI7 V2 架构设计（Issue #4）` | A1/A2 sessions recorded below; active correction Worker to be recorded at dispatch | A1 completed; A2 Worker / T3 exact-review correction | `docs/4-v2-architecture-candidate@4cd8250` / `worktrees/1649` | `review failed — bounded A2 rework authorized` | One correction Worker may amend only the seven A2 paths; all earlier Workers stopped | Re-derive the closed set, correct seam authority/provenance/dispositions/checkpoints, amend A2, stop; no A3 or implementation |
...
| 2026-08-22 | A2 exact-review architecture correction / `docs/4-v2-architecture-candidate` | Worker / T3; candidate review floor remains T3-par | Claude Code / `claude-opus-5` / high | GPT-5.6 Sol / `xhigh`; Worker session pending dispatch | Known exhausted quota window; no repeat Claude attempt before reset evidence | Same-class fallback required by the observed HTTP 429 session-limit response; no task-class downgrade | Correct the failed `4cd8250` review under [A2 rework dispatch](./A2-REWORK-DISPATCH.md); fresh exact-head dual-axis review required |
...
4. Treat exact-head review target `4cd825028c72d57e23a56f95685d59c75f027e6a` as failed and supersedable only by one bounded A2 amendment under [A2-REWORK-DISPATCH.md](./A2-REWORK-DISPATCH.md). Keep A3 blocked and every prior Worker stopped.
