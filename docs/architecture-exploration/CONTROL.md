...
| `AI7 V2 架构设计（Issue #4）` | Authoring task `01a022de-f781-7d31-9a77-c3ce9ee1ce50`; mechanical-recovery session `7bfa7b54-9b68-4d30-9d63-a9c3870647de`; review-fix session `e91a7cae-2a8b-49fe-a863-c788af4dd90c` | Worker / T3-par authoring; Worker / T1 mechanical recovery; Worker / T3 bounded review fixes | `docs/4-v2-architecture-candidate@80c4a6d` / `worktrees/1649` | `rework — exact-head review failed; replacement writer exclusively assigned` | One active writer at a time; first two Workers stopped; review-fix Worker may touch only the five A1 paths plus three named candidate domain/ADR paths | Correct the eight exact review findings, amend one local candidate commit, stop; no A2/A3 entry |
...
| 2026-08-22 | A1 exact-review fixes / `docs/4-v2-architecture-candidate` | Worker / T3 bounded architecture-document correction; candidate review floor remains T3-par | Claude Code / `claude-opus-5` / high; session `e91a7cae-2a8b-49fe-a863-c788af4dd90c` | Pending dispatch result | Pending | Pending | Standards: four P2; Spec: one P1, two P2, one P3; no findings accepted as owner choices |
...
2. Keep the stopped original A1 Worker and stopped Claude mechanical-recovery Worker read-only. Give only review-fix session `e91a7cae-2a8b-49fe-a863-c788af4dd90c` temporary write authority over the eight named candidate paths, starting from `80c4a6d514e351717bd35f0729c2fe7f91ded16d`.
3. After that Worker amends and stops, review its new exact A1 head, confirm the invariant list, and present only the decision-ready platform consistency choices to the owner.
