...
[ADR 0083](./0083-run-every-agent-s-electron-journeys-on-the-background-test-guests.md) has an agent obtain Local completion on a background test guest through the host runner. The runner's working record stays outside every repository and is cited by nobody; "no proof artifact or payload is retained" continues to govern what the repository, the Issue and the pull request carry.
