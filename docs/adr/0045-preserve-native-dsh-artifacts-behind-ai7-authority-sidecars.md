...
### Exact sidecar successor for Issue #165

Issue #165 refines only the AI7 compatibility/authority sidecar state for that unchanged Issue #88 carrier. The native identity, version, bytes, digest, source, installed state and native Book enablement remain unchanged, and the one-revision native-artifact allowlist does not become an artifact-update mechanism. AI7 materializes one stable sidecar identity with two independently inspectable immutable revisions, each carrying that stable identity, one exact revision/version identifier and its exact digest:

- **Revision 1** faithfully records the already-reviewed Issue #88 state: only the Main Editorial Role requirement, with empty AI7 Capability, readable-scope, Provider Binding, credential, network, Effect, Background Analysis Enrollment and Apply authority.
- **Revision 2** differs only by making two current-Book scope kinds eligible to be requested: the exact current Book's primary Manuscript Revision, including exact task-bound Manuscript pins; and exact Source Versions already owned by that same current Book. It retains the Main Editorial Role as its sole Model Role requirement and retains every other Revision 1 authority category as empty: no AI7 Capability or Capability Grant, artifact-selected Provider Binding or fallback, credential access, network access, external research, Cross-project or Series scope, Effect class, Background Analysis Enrollment, Apply authority or artifact code execution.

The later separately authorized additive migration from current schema v12 must materialize Revision 1 from the existing immutable installation metadata, preserve every existing native installation and Book enablement, append an exact Revision 1 sidecar pin for every already-enabled Book, and never rewrite or delete that historical pin. Revision 2 is then presented as an exact successor with its changed eligibility consequences. An already-enabled Book remains pinned to Revision 1 until the editor explicitly reviews and enables Revision 2 for that exact Book; that action appends the Revision 2 pin rather than rewriting history. A fresh installation may review and enable directly against Revision 2. This bounded successor path creates no generic update, reconciliation, `latest`, downgrade, rollback, catalog or multi-revision framework.

A sidecar revision and Book pin establish only the maximum eligible request. They grant no actual read, Task, Plan, Run, Provider processing or execution authority. Every exact readable record remains excluded until a later newly user-initiated Task, Task Input revision and pins, Run Source Scope, Plan Envelope and standard direct Run Authorization independently select and freeze it, while Provider Processing Policy remains separately controlling and default-deny. This refinement authorizes no current schema, protocol, UI, Journey or product change; those require the separately bounded predecessor implementation.
