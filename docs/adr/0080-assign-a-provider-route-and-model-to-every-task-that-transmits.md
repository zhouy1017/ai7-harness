...
### 2 · The profile capability set gains `toolCalling` and `webSearchTool`, and a binding declares its context size

Three proposals, all following the table's own rule that an unverified capability is declared absent:

1. **`toolCalling: 'none' | 'function'`** joins `ModelCapabilities`, declared per `${route}/${model}` with evidence exactly as every other capability is: whether the model, on the request shape its route speaks, accepts function definitions and returns calls the client executes (`tools[].type: "function"` / `tool_calls` / `role: "tool"` on OpenAI chat completions; `tools` / `tool_use` / `tool_result` on Anthropic messages; `function_call` / `function_call_output` on Responses; `functionDeclarations` / `functionCall` / `functionResponse` on Gemini). This is the capability every AI7 platform tool of §7 rides on. Evidence read 2026-09-11: DeepSeek official documents function tools with `strict` JSON-schema parameters and states "From DeepSeek-V3.2, the API supports tool use in the thinking mode" (api-docs.deepseek.com/guides/tool_calls); the `tool_choice` values and the thinking-mode round-trip rules are not on that page and belong to the first live item. `opencode-go/deepseek-v4-flash` is `none` until an item establishes it — the Owner's own session of §5.3 observed `deepseek-v4.1-flash` emitting a well-formed client tool call through the Go gateway on the Anthropic shape, which is the reading the item must repeat on the record.
2. **`webSearchTool: 'none' | 'provider-tool'`** — a **server-side** search tool of the model's own provider (Claude's `web_search_*`, OpenAI's `web_search`, Gemini's `google_search`, Kimi's `$web_search`, GLM's / ERNIE's / MiMo's `web_search`, Hunyuan's `web_search_options`), declared with the vendor page and the day. There is no `platform-tool` value: AI7's own search tool is not a property of any binding — it is available on every binding whose `toolCalling` is `function` — so it is declared in the policy rule (§7.5), never in the profile. The applicable Provider Processing document's `webSearchToolAllowed` flag enables search for a rule by either supply, and the Egress Gate's `tools-present` refusal narrows as §7.2 states.
3. **A declared context size** per profile (input tokens, `vendor-documentation` evidence), so Provider Preflight can compare the plan's declared largest transmission with the binding's declared context and refuse a binding that cannot carry it. §3 records each candidate model's context window where the pages read state one, and "context not read" where they do not.

None of the three authorizes a call: a configured route stays unbindable until a policy revision names its exact binding, and every first transmission on a new binding is a named test item under ADR 0067/0070.
...
Read 2026-09-10 (opencode.ai/docs/go/ · opencode.ai/docs/zen/) and 2026-09-11 (opencode.ai/docs/tools/; the opencode source, `anomalyco/opencode` branch `dev`, `packages/opencode/src/tool/{websearch,webfetch,mcp-websearch}.ts`; and the Owner's own Claude Code session of §5.3):
- The Go plan is "a low cost **$10/month subscription**"; Zen is a separate optional pay-as-you-go gateway. The Go page prints `https://opencode.ai/zen/go/v1/` with `/chat/completions`, `/messages` and `/responses`, the `x-opencode-session` header, and says the models can be reached from third-party SDKs; the Zen page prints `/responses` (GPT, Grok), `/messages` (Claude, Qwen), `/chat/completions` (DeepSeek, MiniMax, GLM, Kimi) and `/models/[id]` (Gemini). Neither page prints a capability table, and **neither mentions tools, web search or web fetch anywhere**.
- **The gateway wire API carries no search tool; the `websearch` / `webfetch` pair belongs to the opencode agent, not to the gateway.** The tools page documents them as built-in tools of the opencode client, governed per tool by `opencode.json`'s `permission` field (`allow` / `deny` / `ask`), enabled by default "only when using the OpenCode or OpenCode Go provider, or when either the `OPENCODE_ENABLE_EXA` or `OPENCODE_ENABLE_PARALLEL` environment variable is set" — conditions of the client process, not of the gateway. The source confirms it: `websearch` is one JSON-RPC `tools/call` from the client to a **public hosted search MCP** — Exa at `https://mcp.exa.ai/mcp` (optional `?exaApiKey=`) or Parallel at `https://search.parallel.ai/mcp` (optional `Authorization: Bearer PARALLEL_API_KEY`) — over HTTP POST with `Accept: application/json, text/event-stream`, taking the first text content block back, 25 seconds, no retry, the backend chosen by `OPENCODE_WEBSEARCH_PROVIDER`, the two flags, or a checksum of the session id; `webfetch` is the client's own HTTP GET of the page with HTML-to-Markdown conversion, a 5 MB cap, a 30-second default and 120-second maximum timeout, and one User-Agent retry on a Cloudflare 403. Exa's page (exa.ai/docs/reference/exa-mcp): "Exa MCP's free plan covers casual use. Add your own API key to lift the rate limits and use it in production", tools `web_search_exa` / `web_fetch_exa`, "works with most other MCP clients". Parallel's page (docs.parallel.ai/integrations/mcp/search-mcp): "no API key required, great for exploration and light use", tools `web_search` (`objective`, `search_queries[]`) and `web_fetch` (`urls[]` up to 20, `full_content`), results capped at roughly 25,000 characters, anonymous searches in `fast` mode with per-`session_id` rate limiting. Neither page states what it retains of an anonymous query.
- **Embeddings**: not stated on any page read.
- **Auth**: the gateway pages name no header form; the Go page prints only `x-opencode-session` for session stability. The repository's own frozen request baseline carries the header form the gateway has been observed to accept; the Owner's session of §5.3 reached `/messages` with the Anthropic header form.
...
**`developer-live`** keeps the v5 binding for everything the product's developer path implements today: Main Editorial Role → `opencode-go/deepseek-v4-flash`, slot `opencode-go` (Provider Processing v5, unchanged). The search-dependent rows (8, 9, 10, 15, 17 — the rows whose basis declares search) have two supplies, each admitted by a new Provider Processing revision naming the eligible binding and the supply: **(a)** a binding whose profile declares `webSearchTool: 'provider-tool'`, or **(b)** the AI7 platform tools of §7 on a binding whose profile declares `toolCalling: 'function'` with the rule's `platformTools` declared. An OpenCode binding is supply (b) — the gateway carries no tool of its own (§3) — and so is DeepSeek official. Until that revision exists, search-dependent rows run provider-free with their disclosed states (S69's 未联网核查 for the review categories; the sibling slices name their own). Row 18's web use rides the same decision — the default-allowed tier permits web use, it does not require it.
...
- Search rows (8, 9, 10, 15, 17): capability-matched among the supported set's bindings that satisfy the row by either supply — `webSearchTool: 'provider-tool'`, or `toolCalling: 'function'` with the AI7 platform tools of §7 — on the Main Editorial Role, with `webSearchToolAllowed` enabled and the supply named for exactly that rule by a Provider Processing revision. Where a binding offers both, the rule says which the plan freezes.
...
#### 5.2 The OpenCode `websearch` / `webfetch` decision — as first read, and as corrected
...
The first proposal of this record read the pair as a *platform tool* that an OpenCode binding carries on the wire. The Commander's review of 2026-09-11 (§3, and the Owner's own observation in §5.3) corrected that reading: the pair is two client-side tools of the opencode agent whose search backend is a public hosted MCP, and the gateway AI7 calls carries neither. **The Owner's judgment stands — the pair does meet the requirement — but it is met by AI7 registering the same two tools in its own harness (§7), which then serve every binding whose model calls functions, OpenCode's and DeepSeek official's alike.** The requirement the pair meets is therefore an AI7 platform capability, not an OpenCode one.

- **How it is read with ADR 0079 §4.2.** That clause says search is the model's tool, never a local search-engine call, and that AI7 calls no search API and holds no search credential. Under §5.3 the clause is read as: **search is model-initiated — the model emits the tool call, and AI7 never composes a query of its own — while AI7 may execute that call on the model's behalf by forwarding it to the public search service the policy names, holding no search credential and building no search-engine integration of its own.** The Owner's privacy sentence is unchanged and binds: the manuscript's text is never published verbatim to a public-access network. A search query derived from a Task does leave the host to the named search service; §5.3 records the Owner's decision on that.
- **Consequences carried into the slices.** S70's Brief names the exact supply of each binding in the plan's 发送 line (EGR-005) and in the policy revision that admits it; the retention fetch stays AI7's own — under §7 it is the same `webfetch` owner, not a second implementation.

#### 5.3 The Owner's decisions of 2026-09-11, second session — the platform tools, the search service, the bounds

The Commander reviewed the first proposal against the opencode source and documentation (§3) and put three questions to the Owner. The Owner first supplied the observation that settled the mechanism, then answered the three; all verbatim.

**The observation.** 「cc switch的配置中是包含tool search的配置的，而且实际上cc switch中转的opencode go可以正常使用websearch工具」, with a Claude Code session (id `42da5457-a986-4fee-b686-93a886ca19b7`, routed by cc-switch to `https://opencode.ai/zen/go` on model `deepseek-flash`) offered as the proof. Its transcript, read by the Commander: line 19 `ToolSearch` loading the `WebSearch` and `WebFetch` schemas (Claude Code's `ENABLE_TOOL_SEARCH` deferred tool loading — tool discovery, not web search); line 48 a client `tool_use` named `WebSearch` emitted by `deepseek-v4.1-flash`, line 49 its `tool_result` in Claude Code's own "Web search results for query" template; lines 52–61 three client `WebFetch` calls, the first refused with HTTP 403 and the second answering from a public weather endpoint; **no `server_tool_use` and no `web_search_tool_result` block anywhere**. The search ran in the Claude Code client as a function-tool round trip; the gateway forwarded a tool call and nothing more. That is the behavior the Owner asked to reproduce.

**Question 1 — reproduce the client tool loop and re-read ADR 0079 §4.2 as §5.2 states?** 「Q1 按照你建议的方法办，我只需要确保复现类似cc switch + claude code的搜索和其它工具的行为模式」. Consequence: §7 is the design; the profile field is `toolCalling` (§2); ADR 0079 §4.2 is read as §5.2 now states.

**Question 2 — the search service as a second recipient.** Asked whether to prefer provider-side search, to hold a Parallel or Exa key in production so that a counterparty with terms exists, and to name the service in the policy bytes — because an anonymous hosted search service states nothing about what it retains of a query derived from an unpublished manuscript — the Owner answered: 「Q2 我认为不重要，搜索泄露出去的不会是完整全文，而且也不是发布在公网，我们不考虑服务商泄露的风险。所以允许即可」. Consequence: the anonymous public search service is allowed in every scope; no search credential is required; the service is still **named** in the policy rule (host, tool, anonymity) because a recipient is a policy fact, not because of a privacy condition; the outbound category of a query is the binding's own (`public-or-synthetic` today), and the house-people strip of §4.4 already applies to everything the model reads, so no query can carry what the model never saw.

**Question 3 — a bound on searches.** Asked whether a per-Run search cap is acceptable as a safety bound, and shown that a fact check of a 200,000-character manuscript needs hundreds to thousands of searches, the Owner answered: 「Q3，我们需要防止陷入死循环，但是不要默认显式规定预算，完成任务是优先。预算应当是总预算+防止浪费，而不是每一个操作都要有预算限制。」 Consequence (§7.4): **the Run Budget Ceiling is the only budget** and it already scales with the frozen unit count; there is no search budget and no per-operation quota; runaway loops are stopped by circuit breakers that no legitimate Run reaches; waste is prevented by deduplication, caching, result-size caps and fetch fallback. Finishing the task comes first: a Run that reaches its ceiling stops as the ceiling already stops it, with the partial outcome kept and 调整预算并重做 offered (S16 #51).

**Two items the Commander's review had left for acceptance, answered the same day.**

- **The product-side credential file of §5** (an enrollment input to the Protected Secret Store, or a store of its own?): 「Q1 保持不变」 — the record stays as written: the slice that implements the file states in its Brief how it relates to ADR 0067's Protected Secret Store and to the `protectedSecretStoreOnly` byte of the Provider Processing documents; nothing about credentials changes in this record.
- **The DeepSeek identifiers** (`deepseek-v4-flash`, the exact `developer-live` binding of Provider Processing v5, is already served by V4.1-Flash; `deepseek-v4-pro`, the production binding, routes there from 2026-09-14 12:00 Beijing time — §3): 「Q2 先不修改，但是要记住我们的provider模型这些配置不能hardcode，一定要是可配置的状态。」 Consequence, and a rule this record carries forward: **no provider, route, endpoint or model identifier is a source constant.** The exactness ADR 0046 and ADR 0065 require lives in configuration — the Provider Processing documents' pinned bindings, the provider documents of ADR 0073, and the Model Service settings — and the code reads those records and freezes what it read into the Execution Binding. Where a source file today holds such a literal (the launch-policy binding constants, the Egress Gate's route and model constants, the connection identifier the analysis store queries by name, the profile table's literal keys), the literal is a defect of the same kind as a hand-picked binding, and the slice that next touches the owner replaces it with a read of the configured record; the id revision the vendor's notice will force is then a policy-byte review, never a code change. The pinned ids themselves are not moved today.
...
- **Sequencing, stated because it is a real dependency**: the first search-enabled live item needs, beyond this record, (1) the `toolCalling` profile field with its evidence (S55a's generator, or a one-field addition ahead of it), (2) a `developer-live` Provider Processing revision (v7) naming the rule's `platformTools` and enabling `webSearchToolAllowed` — the Owner's byte review — and (3) the platform-tools slice of §7.8: the Egress Gate narrowings, the network-denial allowance set, the tool-result payload source, the two tools, their ledger and cache. The binding can be the OpenCode route that already exists (`opencode-go`, developer key enrolled) or DeepSeek official once its key is enrolled the ADR 0073 §4 way. S70's first live item is a named test under ADR 0070 with its stated question — 「can the bound model complete one `websearch` round trip in thinking mode and answer with sources, and what does the full chain cost」.
...
- A follow-up Issue (#452) lists every provider, route, endpoint and model literal in the product source and routes each to the slice that owns its file, under the no-hardcoding rule of §5.3; S55a's generator (ADR 0073) is the natural first consumer, because the closed unions it emits are exactly where those literals should come from.

### 7 · AI7 platform tools: `websearch` and `webfetch`

The reproduction of the client tool loop the Owner observed (§5.3), as AI7 owns it.

**7.1 What they are.** Two tools registered into the harness's `ToolRuntime` (`@deepseek-ai/dsh-tools`, already composed in `src/service/harness/primary-agent-harness.ts` in `native` mode with `registeredTools: 0`; the agent loop already executes tool calls one at a time). They are registered only for a Run whose plan declares WS on the row and whose policy rule names them; every other Run keeps `registeredTools: 0`. `websearch(query, …)` forwards the model's call to the search service the policy names (Parallel `web_search` at `search.parallel.ai` or Exa `web_search_exa` at `mcp.exa.ai`, one JSON-RPC `tools/call` exactly as the opencode source does it) and returns the result text to the model. `webfetch(url)` retrieves the page's text itself — **this is the retention fetch the External Evidence Retention Procedure already owns** (SRC-013, ADR 0079 §4.3): one owner, one implementation, HTML to Markdown, charset detected from the response (the Owner's session of §5.3 shows a Chinese site coming back mojibake without it), a 5 MB cap and a 30-second default timeout as opencode sets them.

**7.2 The Egress Gate keeps deciding every byte that leaves** — three narrowings of the one gate, no new gate:
- `tools-present` refuses unless the payload's tool set equals exactly the platform tools the rule names for the binding; any other tool, or the tools on a Run whose rule names none, is still refused.
- A third decision, `call-search-service`, beside `transmit-remote` and ADR 0074's `fetch-public-source`: the target host must equal the rule's `platformTools.websearch.host`, the query's outbound category must equal the binding's, and the circuit breakers of 7.4 must be intact; one decision is one single-use ticket, and the forwarder sends with the `nativeFetch` the service captured before installing network denial.
- `webfetch` rides `fetch-public-source`: the target is a URL the model cited or the search returned, bounded by the number of citations (ADR 0079 §4.3) rather than by a host list — that ADR already made the curated institutional-source list a preference given to the model and a post-filter on citations.
- A tool result re-enters the model as a message of a new payload source kind, `tool`, carrying the tool-call id, the source URL, the result's SHA-256 and byte count; today the gate admits only `user` (the Run Source Scope's own text) and `model`. Tool results are never logged; their bytes live in the Research Snapshot Cache (ADR 0074 §3).

**7.3 Network denial** gains a policy-declared allowance set in place of the single host: the model endpoint host plus `platformTools.websearch.host`; `webfetch` targets are reached only through a `fetch-public-source` ticket and the captured `nativeFetch`, and the global `fetch` stays replaced by the denial.

**7.4 Bounds — one budget, breakers against loops, rules against waste** (the Owner's decision of §5.3, Q3):
- **The only budget is the Run Budget Ceiling.** Every model turn of the loop counts against the transmission bound and the ceiling; every search result injected into the context is tokens the ceiling already counts. There is no search budget and no per-operation quota. Because search-enabled rows spend more per unit, Provider Processing v7 may declare a larger per-frozen-unit default for those rules — a byte the Owner reviews — and a Run that reaches its ceiling stops exactly as the ceiling stops any Run today: partial outcome kept, 调整预算并重做 offered.
- **Circuit breakers, set where no legitimate Run arrives:** an identical tool call (same tool, same canonical arguments) repeated within one unit is refused and the refusal is returned to the model as the tool's result; a unit whose consecutive tool round trips exceed the breaker (a large constant the slice fixes and states, not a per-unit budget) ends that unit with the disclosed state 联网核查未完成 on its affected findings, never a retry; `maxParallelToolCalls` stays 1.
- **Against waste:** one query is sent once per Run (keyed by the digest of its canonical arguments) and answered from the Research Snapshot Cache across Runs; a search result is capped at 25,000 characters before it enters the context, a fetched page at 5 MB before conversion; a `webfetch` that fails on one citation moves to the next rather than repeating.

**7.5 Policy bytes.** A Provider Processing revision (v7 for `developer-live`; the production successor when its binding is chosen) names, in the rule:
```
"platformTools": {
  "websearch": { "service": "parallel", "host": "search.parallel.ai", "tool": "web_search", "anonymous": true },
  "webfetch":  { "maxBytes": 5242880, "timeoutSeconds": 30, "boundedByCitations": true }
}
```
`webSearchToolAllowed` stays the rule's switch. The Commander proposes Parallel as the default service (its page states the anonymous use plainly, documents a companion `web_fetch`, and caps results explicitly); the Owner chooses at the byte review. The Owner's Q2 decision means `anonymous: true` is acceptable in every scope.

**7.6 Records.** Each `websearch` call writes a Provider Test Ledger item of its own kind (ADR 0067, ADR 0074's "once"): the query's digest, the service host, the result's digest, the elapsed time — never the text. Results enter the Research Snapshot Cache; fetched pages become Book-owned Source Versions as SRC-013 says, within the Task that retained them.

**7.7 The first live item's question.** 「Can `opencode-go/deepseek-v4-flash` (or DeepSeek official `deepseek-flash`) complete one `websearch` round trip in thinking mode and answer with sources, and what does the full chain cost?」 — a named item under ADR 0070, after the v7 bytes pass the Owner's review.

**7.8 Landing order.** (1) `toolCalling` in the profile set with documentation evidence; (2) Provider Processing v7 bytes (Owner's review); (3) one T3 slice for the gate narrowings, the allowance set, the `tool` payload source and the two tools with their ledger and cache — ahead of S70 or as its first half; (4) the named live item; (5) S70's retention, alignment and translation steps over what the tools return.
...
- **No provider, route, endpoint or model identifier is hardcoded** (§5.3): every one is read from a configuration record — a policy document's pinned binding, a provider document, a Model Service setting — so that a vendor renaming a model, as DeepSeek's notice of §3 does, is answered by a policy-byte review and never by a source edit. The source literals that exist today are listed in the follow-up Issue this record names in §6 and retire as their owners are next touched.
...
- **Treating the OpenCode gateways as search suppliers.** The first proposal read the opencode `websearch` / `webfetch` pair as a capability of the gateway; the source and the Owner's own session (§3, §5.3) show it is a client tool loop over a public search MCP, so the gateway supplies nothing the model does not already do. What the Owner asked for is reproduced instead as AI7's own platform tools (§7), which serve every function-calling binding alike.
- **A per-Run search cap.** Proposed by the Commander as a safety bound and declined by the Owner (§5.3, Q3): a fact check of a long manuscript needs hundreds of searches, and finishing the task comes first; the Run Budget Ceiling remains the one budget, with circuit breakers against loops and rules against waste.
- **A search credential or a provider-side-only rule for privacy.** Proposed by the Commander because an anonymous search service states no retention terms; declined by the Owner (§5.3, Q2): a query is not the full text and is not publication, and the service's own leakage risk is not a design consideration here.
