...
Pure documentation, design, and CI-governance changes may integrate while the workflow remains disabled when their exact Change Brief permits it, after local validation and one Commander-dispatched advisory read-only review. The Reviewer verdict informs the Commander's decision but never becomes a zero-finding, iterative re-review, or merge gate. This exception does not claim a green Gate and creates no required branch-protection status check.

The workflow cutover is a separate CI-governance outcome: start from then-current `dev`, implement only this ADR in the existing workflow, validate its static structure locally, obtain advisory read-only review, and integrate it while the workflow remains disabled. Re-enablement waits for the exact Owner restoration statement. After restoration, queued product branches are processed one at a time in dependency order: re-resolve target authority, rebase onto current `dev`, locally revalidate, make Ready, run one paired-platform Gate occurrence, merge, then advance to the next branch. No synthetic run is dispatched merely to test CI.
