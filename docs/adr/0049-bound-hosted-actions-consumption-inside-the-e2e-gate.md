...
Within each platform occurrence, checkout, exact tool selection, bootstrap, and build happen once before the four Journeys run sequentially. ADR 0053 permits the narrow integrity-reverified Electron download cache and failure early-stop; a successful occurrence still completes all four Journeys on both platforms.
