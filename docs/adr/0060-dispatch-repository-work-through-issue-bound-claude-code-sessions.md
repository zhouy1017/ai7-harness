...
This decision is superseded in full by [ADR 0061](./0061-route-repository-dispatch-by-commander-harness.md). Its Claude Code-only exclusivity is historical; its Claude Code route, bindings, session tooling, and schema-v2 receipt shape continue as the `claude-code` route under ADR 0061 beside the restored Codex route. On 2026-09-04 the Owner reversed two rejected alternatives below: the first, so ADR 0061 now admits a Commander-spawned Agent-tool subagent as a Claude Code launch mode beside the hand-created top-level session, and the fourth, so [ADR 0063](./0063-allow-cross-harness-dispatch-through-cli-launched-task-sessions.md) now admits CLI-launched, cross-harness Task Sessions. The original body remains below unchanged as decision history.
