...
The accepted Electron three-process architecture, ProseMirror bounded editor, TypeScript/Node runtime, Standalone-only scope, Word exclusion, and no-TCP local boundary apply on both platforms. Windows retains its zip-portable and NSIS channels. macOS minimum version and CPU policy, package/update channel, Agent Data Root location, Keychain integration details, signing/notarization, and exact local IPC carrier remain implementation decisions; they must not inherit Windows mechanics silently and do not reopen whether macOS is in product scope.
