...
The list is a semantic coverage map. It does not inherit old screen geometry, prototype/Figma artifacts, component implementation, Windows mechanism detail or completed verification claims. Future implementation evidence may cover these as complete Windows end-to-end user journeys plus discovered bug regressions; this document creates no separate usability, accessibility, performance or UI gate.
...
| **J-12** | Understand Windows distribution/data-location exceptions and configure model processing without exposing secrets | Use installer and portable channels; encounter writable portable, installer-default, unwritable fallback, sync/backup warning and prohibited repository-location states → inspect `设置 > 数据与存储` → perform on-demand Model Service setup by Model Role → replace/remove a credential without revealing it → return to exact Task setup point → distinguish controlled Provider processing from public release | Mapped |
...
| **J-14** | Complete professional Windows work by keyboard and under Chinese/high-contrast/zoom constraints | Traverse J-01–J-13 through labeled keyboard paths at 1366×768 and Windows scaling, Simplified Chinese IME, 200% application zoom/reflow, visible focus, light/dark themes and forced colors → use resized/hidden surfaces and editable Detached Manuscript Window without losing exact context or state meaning | Mapped; behavior retained, no separate gate |
...
Keyboard, IME, focus, scaling/reflow, high contrast/forced colors and low-resolution behavior are unconditional design requirements throughout the journeys. Their inclusion here does not authorize a separate usability/accessibility/performance/UI validation gate.
