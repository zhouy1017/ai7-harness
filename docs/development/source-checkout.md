...
Database open preserves the canonical v1-through-v5 migration history, then admits only an exact canonical v5 database or the exact final v6 structure. The bounded module performs one direct v5-to-v6 upgrade; unintegrated candidate database lineages are not compatibility inputs. DOCX parsing writes at most 256 buffered rows per synchronous short transaction to an inert same-database ingest relation, and a separate short authority transaction either promotes the complete validated snapshot or exposes none of it. Startup removes inert ingest rows before import recovery; the relation is not a second store, recovery owner, or commit authority.
