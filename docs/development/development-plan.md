...
| 1c.5 | S50 | #311 | T2 | J-01, J-04, J-12 | Test manuscripts composed from admitted SampleBook content rather than invented text | #352 | planned; dispatchable beside S51b-1 now that S51a is integrated |
| 1c.6 | S51 | #313 | T3 | J-01 | Multi-format manuscript intake normalizing to DOCX, with the Source Version always keeping the original | #297 | split into four units on the Issue under ADR 0072: S51a #352 integrated (PR #357, `dev@1c5168fc`); S51b-1 #350 (format routing, source-only retention of any recognised file, schema 18; T3) next; then S51b-2 #356 (`.txt` / `.md` conversion), S51c #351 (the `.doc` converter) |
...
| 1c.9 | S54 | #321 | T3 | J-04, J-12 | The explicit provider support list — Gemini, OpenCode Go with every plan model, DeepSeek official, OpenAI, Claude, Qwen, HY — implementing the request shapes #310 named and refused | #310 | split into five units on the Issue: S54a #338 integrated (PR #340, `dev@0794a71`); S54b #342 integrated (PR #348, `dev@fcbaf3a`); S54c #354 integrated (PR #359, `dev@2bab1589`); S54d (`google-generate-content`, T3) next — Gemini has no Go route, so the shape ships against test-local profiles and its route waits for 1c.10's credential slot; then S54e (T1). Claude, Qwen, and HY official endpoints are 1c.10's first format-selected configurations |
