...
| 2.2 | S71 | #406 | T2 | J-04 | ①b / ②A The finished analysis chain in editorial language, including the Run Report (S42b, S44b) | Phase 1, S57 | pull request open (PR #480); the quick start on ②A is shown disabled until S75 |
...
- **Gate health, ahead of every unit below (the fix is open as PR #479: `navigate()` held its paging guard across the S57 position write, which drops J-01's and J-02's paired paging commands on a slow disk; the macOS J-02 failures are not yet explained and now name their step):** #474 (J-02 fails at `j14-behavior` on five of the six hosted nightly platform runs since PR #470 and passes on the developer host) and #475 (J-01 fails on hosted Windows on three of five nights). Under ADR 0081 the queue tests every candidate as the tree that would land, which contains `dev`, so the nightly merges nothing until the nightly over `dev` is green again.
