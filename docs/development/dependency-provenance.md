...
Electron is a secondary runtime artifact, not an npm dependency. Its own `LICENSE` and `LICENSES.chromium.html` are copied from the verified current-host carrier into every `dist/notices/` output alongside the tracked AI7 notices. Node's license source and Electron's license source are declared in the artifact manifest.
