...
- a no-write bootstrap-preflight instruction that reports the generated actual cwd/worktree and branch-or-detached state, carrying in the `subagent` mode the retention-marker instruction the dispatch owner defines: leave exactly one untracked, uncommitted scratch marker `.ai7-preflight-<dispatch_id>` at the worktree root as the preflight's only write, so the harness does not remove an unchanged worktree before the attach step, and delete it after verifying the Launch Receipt.
