...
`main` is the only canonical design line. `design-doc` is an aggregate exception for consolidated reading and review. A file being visible, newer, longer, merged, review-clean, frozen, or internally labeled `accepted-candidate` does not make it canonical.
...
| **Worker** | Execute the exact brief in its own worktree and branch; stop and report when the envelope is insufficient | Expand scope, self-authorize replacement, integrate, push to `main`, publish, or take external actions |
