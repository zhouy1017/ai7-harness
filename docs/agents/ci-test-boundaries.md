...
ADRs 0055 and 0056 keep admission distinct from executable activation. Issue #88 made the current real executable set J-01, J-02, J-08, J-12 and J-15; J-03 remains admitted but dormant. Under ADR 0057 the active workflow contains only those five real commands. Issue #47 later supplies the real J-03 runner/dispatcher and atomically adds J-03 to local `e2e:all` and the hosted workflow. A product-affecting Ready pull request runs every then-current executable admitted Journey on both Windows and macOS; shared and unclassified changes fail closed to that same set. No phase fixes a permanent total, and every further admission still requires an explicit Owner routing decision and separate CI-governance integration.
...
Resolve those IDs from the exact head. The current set is J-01/J-02/J-08/J-12/J-15; it includes J-03 only after Issue #47's later atomic cutover. Never disclose a dormant Journey as passed or assert a fixed total.
...
`build` validates and replaces the canonical build output; `e2e:all` starts every then-current executable admitted Journey in a fresh controller process and stops at the first failure. The order is now J-01 → J-02 → J-08 → J-12 → J-15; Issue #47 later atomically adds real J-03. Resolve every locally reproducible failure. Report only exact head, supported host, commands, and outcomes; retain no log, receipt, payload, database, screenshot, trace, video, or proof artifact.
...
Resolve the disclosure list from the exact head: the current five include J-15 while J-03 remains dormant, and J-03 is included only after Issue #47's later cutover. A dormant command is never a completion result, and no report asserts a fixed total.
