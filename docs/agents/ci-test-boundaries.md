...
ADRs 0055 and 0056 keep admission distinct from executable activation. Issue #88 made J-01, J-02, J-08, J-12 and J-15 executable; Issue #47 now supplies the real J-03 runner/dispatcher and atomically adds it to local `e2e:all` and the hosted workflow. The active workflow contains those six real commands. The Owner's 2026-09-05 directive admits the bounded S36 covered-baseline-analysis slice of J-04 owned by Issue #92 as a further executable admitted Journey; J-04 stays dormant and absent from local `e2e:all`, `ADMITTED_JOURNEYS`, and the hosted workflow until Issue #92 supplies its real dispatcher and runner and atomically cuts it over, after which Local completion and every Hosted Gate occurrence execute every then-current real admitted Journey, including J-04. No alias, fake success, skipped J-04 completion, or fixed permanent total may bridge that cutover. A product-affecting Ready pull request runs every then-current executable admitted Journey on both Windows and macOS; shared and unclassified changes fail closed to that same set. No phase fixes a permanent total, and every further admission still requires an explicit Owner routing decision and separate CI-governance integration.
...
Resolve those IDs from the exact head. The current set is J-01/J-02/J-08/J-12/J-15/J-03 after Issue #47's atomic cutover, and gains J-04 only at Issue #92's cutover. Never disclose a dormant Journey as passed or assert a fixed permanent total.
...
Resolve the disclosure list from the exact head: the current set includes J-15 and J-03 after their atomic cutovers, and gains J-04 only at Issue #92's cutover. A dormant command is never a completion result, and no report asserts a fixed permanent total.
