...
ADR 0054 changes only when macOS integration evidence is required. Before the exact Initial v1.0.0 Development Milestone Boundary, Windows Local completion is the only required platform evidence; that result is not a Windows-only Gate, paired-platform evidence, or a macOS pass. The same-product Windows/macOS contract, semantic parity, admitted Journeys, and dormant paired workflow topology remain unchanged.
...
A pull request remains Draft during authoring, debugging, review, rebase, and local validation. Draft activity runs no hosted workflow. Once the change is locally complete and target authority has been re-resolved, only the Commander changes it to Ready for review. After successful post-boundary re-entry, when the exact workflow has been separately restored and is applicable, that transition starts the one logical Gate. Before the ADR 0054 boundary, Ready follows the independent Windows Local-completion route and does not start disabled workflow `342459594`. A later push while an applicable workflow occurrence is running starts the newest occurrence and cancels any superseded in-progress occurrence for that same pull request; cancellation is a consumption control, not exact-head or same-SHA proof.
...
ADR 0055 admits Issue #47's bounded J-03 slice as the next supported Journey decision but keeps admission distinct from executable activation. The current executable admitted set remains J-01, J-02, J-08 and J-12 until another separately authorized Journey implementation changes it. Issue #88's required but unresolved J-15 routing decision may do so before Issue #47. Issue #47 must then add the real J-03 runner/dispatcher and atomically add J-03 to the then-current local orchestration. A normal later product-affecting Ready pull request runs every then-current executable admitted Journey on both Windows and macOS; shared and unclassified changes fail closed to that same set. ADR 0054 changes only the required evidence platform before its boundary, not the set or post-boundary topology. Every further admission still requires an explicit Owner routing decision and separate CI-governance integration.

Each applicable platform occurrence checks out and selects the exact toolchain once, performs bootstrap and build once, then runs every then-current executable admitted Journey sequentially. The workflow may restore only `.cache/bootstrap/electron`, keyed by OS, architecture, and the complete declared artifact manifest. Bootstrap revalidates its canonical path and SHA-256; a miss follows the same path, and only a successful bootstrap may save the verified download for a later occurrence in the same pull request. Do not assume cross-pull-request reuse. A hit has no success meaning. Never cache `.pnpm-store`, `node_modules`, `.runtime`, `dist`, Agent Data Roots, databases, product runtime state, manuscript-shaped material, credentials, logs, screenshots, traces, videos, or test artifacts. Matrix failure early-stop may cancel the sibling after a definitive failure; only both platforms completing the whole resolved set is a successful Gate. The disabled workflow may project J-03 after J-12 before Issue #47 supplies its runner, but that dormant command may not execute or become evidence.
...
An admitted scenario has no quarantine, flaky registry, tolerated-failure status, or platform waiver. A failure or unknown on a platform currently required by ADR 0054 blocks integration and is fixed in the product or scenario. Before the boundary, a known macOS-only issue remains an unresolved re-entry obligation rather than a pass, tolerated failure, or pre-boundary blocker. After successful re-entry, either platform failure again blocks the applicable lifecycle. Remove or replace a scenario only when the supported journey or recorded bug outcome has explicitly changed; do not hide a failure behind standing exception machinery.
...
Normal pull-request flow and Commander-only external action/integration remain in force. An implementation change that affects a supported Journey or an observed-bug outcome updates the applicable E2E scenario and completes the local loop. ADR 0054 independently permits pre-boundary integration from fresh Windows Local completion regardless of whether ADR 0053's external Hosted-CI condition remains active. When that condition is active, ADR 0053's workflow-state, no-run, and truthful external-condition records also apply. After successful post-boundary re-entry, the normal lifecycle again receives one integration-ready occurrence of the logical Gate on both platforms, or uses ADR 0053 when its condition is active. A blocking failure or unknown under the current milestone phase never enters either route. Documentation-only and design-only changes do not invent an automated proof task. Independent Reviewer work is optional and advisory under [Repository Development Dispatch](../../kick-in/27-repository-development-dispatch.md), not a prerequisite for the pull request or the Gate.
...
Every Windows product, bootstrap, build, or Journey failure or unknown cause blocks Ready and merge. This Windows-only evidence requirement applies even when the diff affects shared or macOS-native code. Missing macOS evidence and any known macOS-only problem are recorded for mandatory re-entry, not represented as passing and not made a pre-boundary merge requirement. Product pull requests integrate one at a time; the next candidate rebases and reruns the same Windows sequence.

ADR 0054 supplies this route independently of ADR 0053. When ADR 0053's external condition is active, its exact workflow-state, absence-of-runs, and external-reason checks apply in addition. If that condition resolves before the boundary, ADR 0054 remains active and Windows Local completion still supplies the authorized integration route; the paired workflow remains disabled and unrun, and no Hosted occurrence or Gate result is claimed.

Exact workflow `E2E Functional Gate` (ID `342459594`) remains `disabled_manually` and unrun. Its existing Windows/macOS matrix stays dormant and structurally unchanged for post-boundary use. Restoration, dispatch, rerun, probing, or replacement before the boundary requires another exact Owner and CI-governance decision.

Each product pull request integrated before the boundary states exactly, with placeholders resolved:
...
Resolve those IDs from the exact head. They remain J-01/J-02/J-08/J-12 until another separately authorized executable cutover, may include J-15 only after its own unresolved decision and implementation, and must include J-03 after Issue #47. Never disclose a dormant projection as passed.

At the exact boundary commit, this exception expires. Before any later product pull request integrates—and before any `dev` to `main` promotion, `v1.0.0` tag, package, signing, notarization, publication, or release action—a separate authorized CI-governance/re-entry Issue validates every then-current executable admitted Journey on integrated `dev` using actual Windows and macOS, or through the exact paired workflow if restoration is separately authorized. Re-entry resolves the set at that exact commit and includes J-03 only after Issue #47's cutover; it fixes no total and does not decide J-15. With Hosted CI unavailable, those are two Local completions rather than a Hosted or green Gate. Either platform failure blocks re-entry and is fixed under its own authorized product scope. Ready but unmerged product pull requests rebase after successful re-entry and resume the normal current Windows/macOS lifecycle. Re-entry validates consolidated `dev`; it does not retrospectively relabel or backfill old pull requests.

## CI-degraded operation

The Owner suspended GitHub-hosted Actions testing on 2026-08-28 after confirming exhausted usage. Exact workflow `E2E Functional Gate` (ID `342459594`) remains disabled; Issue #138 does not authorize enablement or dispatch.

The ADR 0053 path is active only when the exact Hosted Gate cannot produce a paired-platform occurrence because of a Commander-recorded external CI condition unrelated to current product source/bootstrap/build/Journey behavior, workflow `342459594` is `disabled_manually` with no queued or in-progress run, and no product/bootstrap/build/Journey failure or unknown that the current ADR 0054 milestone phase makes blocking remains unresolved. Before the boundary, every Windows failure or unknown is blocking while a known macOS-only problem follows ADR 0054's truthful re-entry record. A date, expected reset, slow queue, visible control, or ambiguous failure does not activate the path. Immediately before Ready and again before merge, the Commander records the exact workflow state, absence of runs, concrete external reason, current milestone phase, and applicable disclosure. No workflow is enabled, dispatched, rerun, or replaced to probe availability or manufacture evidence.
...
`build` validates and replaces the canonical build output; `e2e:all` starts every then-current executable admitted Journey in a fresh controller process and stops at the first failure. The current order is J-01 → J-02 → J-08 → J-12 until another separately authorized executable cutover. Issue #88's J-15 route may change it only through its own decision; Issue #47 must add real J-03 to the resulting orchestration. Resolve every locally reproducible failure. Report only exact head, supported host, commands, and outcomes; retain no log, receipt, payload, database, screenshot, trace, video, or proof artifact.
...
Resolve the disclosure list by the same phase rule: current four until another separate cutover, optional J-15 only through its unresolved authority route, and mandatory J-03 after Issue #47. A dormant command is never a completion result.

Before the Initial v1.0.0 Development Milestone Boundary, authoritative resolution of the external hosted condition neither ends ADR 0054 nor authorizes workflow action. At the boundary, the exception expires and the separately authorized re-entry above must complete before later product integration. After re-entry, every Ready but unmerged product pull request rebases and returns to ADR 0049's normal paired-platform lifecycle. Availability confirmation does not authorize restoration: workflow enablement or dispatch requires separate exact external-action authority. There is no automatic restoration, probe, manual dispatch, retrospective run, synthetic backfill, or per-pull-request backfill; the next normal Gate exercises then-current integrated `dev`.
