...
Under ADR 0051, a normal product-affecting Ready pull request runs all currently admitted J-01, J-02, J-08, and J-12 journeys on both Windows and macOS. Shared and unclassified changes also fail closed to all four journeys. ADR 0054 temporarily changes the required evidence platform before its exact boundary without changing this Journey set or the Gate's post-boundary topology. Admission of any later supported journey requires another explicit Owner routing decision and separate CI-governance integration; it never silently changes this set.

Each platform occurrence checks out and selects the exact toolchain once, performs bootstrap and build once, then runs the four Journeys sequentially. The workflow may restore only `.cache/bootstrap/electron`, keyed by OS, architecture, and the complete declared artifact manifest. Bootstrap revalidates its canonical path and SHA-256; a miss follows the same path, and only a successful bootstrap may save the verified download for a later occurrence in the same pull request. Do not assume cross-pull-request reuse. A hit has no success meaning. Never cache `.pnpm-store`, `node_modules`, `.runtime`, `dist`, Agent Data Roots, databases, product runtime state, manuscript-shaped material, credentials, logs, screenshots, traces, videos, or test artifacts. Matrix failure early-stop may cancel the sibling after a definitive failure; only both platforms completing all four Journeys is a successful Gate.
...
`build` validates and replaces the canonical build output; `e2e:all` starts each existing Journey in a fresh controller process, in J-01 → J-02 → J-08 → J-12 order, and stops at the first failure. Resolve every locally reproducible failure. Report only exact head, supported host, commands, and outcomes; retain no log, receipt, payload, database, screenshot, trace, video, or proof artifact.
