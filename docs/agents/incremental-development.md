...
The hosted E2E occurrence is integration evidence rather than a debugger. A locally reproducible hosted failure returns the pull request to Draft for local repair. Only the Commander may rerun one clearly external runner/network/infrastructure transient without a code change. During the exact hosted-execution suspension, documentation, design, and CI-governance work follows the narrow path in the CI boundary. Product work normally remains unmerged, but an otherwise-ready product pull request may integrate one at a time without hosted evidence while the exact ADR 0050 waiver is current and active. The Commander records its pre-Ready and pre-merge conditions and exact disclosure; no local result, advisory review, fake green, substitute, or single-platform run is claimed as the Gate. Confirmed reset ends the waiver immediately and returns every Ready but unmerged product pull request to the normal paired-platform lifecycle.
