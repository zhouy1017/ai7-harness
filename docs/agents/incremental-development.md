...
Before the exact Initial v1.0.0 Development Milestone Boundary, fresh exact-head Windows `doctor` → `bootstrap` → `build` → `e2e:all` is required before a product pull request becomes Ready, including for shared and macOS-native changes. While the workflow restored under [ADR 0057](../adr/0057-restore-hosted-gate-under-observed-actions-usage.md) is active and usable, the ensuing paired Hosted Gate must also succeed before merge. During an exact [ADR 0053](../adr/0053-preserve-local-first-development-through-a-bounded-ci-degraded-mode.md) degraded interval only, [ADR 0054](../adr/0054-defer-macos-evidence-until-after-initial-v1-0-0-development-milestone.md) supplies the Windows-only integration route with truthful macOS deferral. No route bypasses a required product, bootstrap, build, or Journey failure or unknown. Product changes integrate one at a time from current `dev`. Independent modules may proceed in parallel only when they consume stable owners and interfaces on current `dev`. Do not silently stack downstream work on unintegrated candidate code; a genuinely necessary stacked dependency must be explicit in both Change Briefs, separately authorized, and assigned an integration order.
...
The Hosted E2E occurrence is integration evidence rather than a debugger. A locally reproducible hosted failure returns the pull request to Draft for local repair. Only the Commander may rerun one clearly external runner/network/infrastructure transient without a code change. The Commander observes authoritative Actions usage before Ready and after the normal occurrence; an unreadable or insufficient meter leaves the pull request Draft and never authorizes a probe. During exact CI-degraded operation, documentation, design, and CI-governance work follows its Change Brief's applicable local path; product work follows ADR 0053, ADR 0054's current milestone phase, and the CI boundary. The Commander records the pre-Ready and pre-merge activation facts and exact disclosure; no local result, advisory review, fake green, substitute, or single-platform run is claimed as the Gate.
