...
While the exact CI-degraded conditions in that boundary are active, Commanders may push task branches and maintain Draft pull requests. A product pull request may become Ready and merge without hosted evidence only while the Commander has recorded every exact activation condition in the CI boundary; no unresolved or ambiguous product/bootstrap/build/Journey failure is eligible. Integrate one product pull request at a time, honor its affected-platform Draft stop, and use the CI boundary's exact ADR 0053 disclosure. Local completion and every authority, privacy, dependency, and Commander-only rule remain unchanged; no hosted run, paired-platform evidence, green Gate, substitute Gate, or single-platform Gate is claimed. Pure documentation, design, and CI-governance work follows its Change Brief's applicable local validation; optional advisory read-only review remains non-gating.

Authoritative resolution of the recorded external condition immediately returns a Ready but unmerged product pull request to the normal paired-platform Gate lifecycle. Workflow `342459594` restoration or dispatch requires separate exact external-action authority. Do not probe, manually dispatch, automatically enable, replace, or retrospectively run the workflow.
