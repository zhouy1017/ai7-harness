...
While the exact hosted-execution suspension in that boundary is active, Commanders may push task branches and maintain Draft pull requests. Once ADR 0050 is integrated, an otherwise-ready product pull request may become Ready and merge without hosted evidence only while workflow `342459594` is recorded `disabled_manually`, no run is queued or active, and no fresh usable Actions allocation after reset has been authoritatively confirmed. The Commander records those facts immediately before Ready and again before merge, integrates waived product pull requests one at a time in authorized dependency order, and includes ADR 0050's exact disclosure in each waived pull request. Local completion and every authority, privacy, dependency, and Commander-only rule remain unchanged; no hosted run, green Gate, substitute Gate, or single-platform Gate is claimed. Pure documentation, design, and CI-governance work may use the expressly authorized disabled-workflow integration path after local validation; optional advisory read-only review remains non-gating.

Authoritative confirmation of a fresh usable Actions allocation after reset expires the waiver immediately. A Ready but unmerged product pull request returns to the normal paired-platform Gate lifecycle. Only after that confirmation may the Commander use ADR 0050's prospective Owner authority to re-enable exact workflow `342459594` as a separate explicit action. Do not probe, manually dispatch, automatically enable, or retrospectively run the workflow.
