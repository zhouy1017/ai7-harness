...
- Body records only the Change Brief closure delta: planned versus actual structure, existing implementation reused, any new owner/dependency and why, journey/bug outcome or `N/A` for non-behavior work, the Local Verification Ladder attestation (exact head, host, outcome of each available layer) for product work, migration/cleanup, unresolved matters, and archive-sweep result when a lifecycle node was triggered.
...
- Independent review is optional and advisory. When review is requested, the Commander directly launches a fresh top-level, strictly read-only, non-author Reviewer Task at the immutable completed authoring commit recorded as `reviewed_head`; a Reviewer may not dispatch, delegate to, or spawn another agent. Record the reviewed task class and use that class's fixed binding on the Commander's harness.
