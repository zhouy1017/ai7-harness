...
This Issue #86 revision is repository-current only when read from an exact integrated `dev` commit containing it; elsewhere it remains accepted-but-unintegrated. It specifies future product seams and does not claim they exist in the current provider-free tracer.
...
This accepted specification selects responsibilities and seams and retains ADR 0020's exact core DSH `0.1.0-rc.6` baseline. It does not select the final package list, catalog-source set, artifact/Plugin, sidecar or persistence schema, DSH carrier mapping, sandbox, provider endpoint, credential, trusted launch-selector implementation, or package layout. The current code remains the provider-free J-01 tracer with zero Providers/Agents/Sessions, atomic import, `r1`, a bounded first window, and durable journal acknowledgement; it implements none of the artifact lifecycle, Provider/egress, retrieval/Exact Fetch, covered analysis/result, Enrollment/metric, or AI7 Apply design above. In particular, `src/service/store.ts` still seeds an AI7 `WORKFLOW_PROFILE` and persists `workflow_profiles.definition_json`; that is current tracer legacy and a future authorized migration gap, not implementation evidence for the successor native-definition/AI7-projection ownership. Exact action authority comes from the applicable Owner instruction, Issue, and Commander dispatch; no design statement permits adjacent source copying, dependency/artifact installation, GitHub search, Provider call, manuscript handling outside the standing exceptions, implementation Issue decomposition, PR/integration, external action, release, or promotion to `main`.
