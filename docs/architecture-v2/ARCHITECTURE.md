...
This accepted specification selects responsibilities and seams and retains ADR 0020's exact core DSH `0.1.0-rc.6` baseline. It does not select the final package list, catalog-source set, artifact/Plugin, sidecar or persistence schema, DSH carrier mapping, sandbox, provider endpoint, credential, trusted launch-selector implementation, or package layout. The current code integrates the provider-free J-01 new-Book path with exact identity disclosure and interrupted-import continuity/reconciliation, complete provider-free J-02 10M bounded editing, and complete provider-free J-08 Recovery Workspace. It remains neither full J-01 nor the broader Book Workspace, has zero Providers/Agents/Sessions, and implements none of the artifact lifecycle, Provider/egress, retrieval/Exact Fetch, covered analysis/result, Enrollment/metric, Proposal/Effect, or AI7 Apply design above. In particular, `src/service/store.ts` still seeds an AI7 `WORKFLOW_PROFILE` and persists `workflow_profiles.definition_json`; that is legacy current-code reality and a future authorized migration gap, not implementation evidence for the successor native-definition/AI7-projection ownership. Exact action authority comes from the applicable Owner instruction, Issue, and Commander dispatch; no design statement permits adjacent source copying, dependency/artifact installation, GitHub search, Provider call, manuscript handling outside the standing exceptions, implementation Issue decomposition, PR/integration, external action, release, or promotion to `main`.
