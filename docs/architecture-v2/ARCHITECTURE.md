...
Status: **frozen design reference as of `dev@4c50ce31b0f15ff2bfadd2af17fc914c317e0f22` under [ADR 0064](../adr/0064-reweight-repository-development-toward-value-first-delivery.md); product and component boundaries; action authority remains Issue-bound**
...
Windows retains its zip-portable and NSIS distribution adapters. The selected macOS V1 adapter is macOS 15+ on Apple Silicon arm64 with bundle `io.github.zhouy1017.ai7`, delivered as a direct-download Developer ID-signed, hardened-runtime, notarized DMG and updated by manual application replacement that preserves product data. Source-checkout execution reports its actual form and performs none of those packaging or release actions.
...
- Editorial Deliverables and immutable revisions, AI7-facing Workflow Profile projections/selectors, durable Workflow Instances, phase/gate state, artifacts, signoffs, the Manuscript's Milestone and Publication Versions, Production Document versions and Delivery Records, the Book-level delivery package, and versioned Maintenance Cases (ADR 0077);
...
The main process is deliberately thin. It creates windows, owns native application lifecycle, one serialized exact-Book Workbench registry, main-owned Product Data Location reveal, user-chosen file and destination dialogs, and native rename/cancel/replace conflict presentation; it supervises the Node service and exposes only a small typed IPC bridge. At most one full Book Workbench exists per exact Book, distinct Books may coexist, and only an explicit editor route may create, reveal, or focus one; background work/state never does. It returns normalized user choices to the service but owns no manuscript model, Task Ledger, agent loop, provider secret, Local Export Preparation, Effect Approval, commit, reconciliation, receipt, or other Effect semantics.
...
All product persistence sits under AI7-controlled locations inside one canonical Agent Data Root shared by the application/service instance and all windows except the Protected Secret Store, and this includes governed manuscript derivatives, the Harness Session Ledger, Book-bound analysis results, and the AI7-controlled native-artifact store. Application binaries and OS-disposable state are outside it. For a buildable launch, bootstrap creates a manifest-bound, digest-bound snapshot of required core runtime dependencies; before the product no-network interval, the launch path atomically materializes and re-verifies it into the selected Agent Data Root. A prefilled global store or another development root never supplies correctness. On Windows, the portable channel keeps data inside the AI7 folder and the installer normally uses `%LOCALAPPDATA%\AI7`. On macOS 15+ arm64, the root is the OS-resolved per-user Application Support location for `io.github.zhouy1017.ai7`, never beside the `.app`, with no silent fallback. Secrets remain outside copied product data and are resolved through Windows Credential Manager or macOS Keychain behind the same Credential Broker. Manuscripts, analysis results/indexes, and their other derivatives never enter repositories, hosted CI, build artifacts, or shipped fixtures.
