...
This is a bilingual reference index and collision guide for the terms created by the exact owner resolutions registered in A1 and the later U2 owner resolution admitted in A2. It follows the discipline of the canonical [Glossary Reference](../../GLOSSARY.md): English terms are stable architecture identifiers, the Simplified Chinese column is the preferred Chinese-first label, and **definitions live in the owning context file, never here**.
...
| --- | --- | --- | --- |
| Primary Agent Harness | 主代理执行框架 | [Execution (V2 candidate)](./domain/execution/CONTEXT.md) | A closure pass for one exact Codex surface; the current A2 verdict is `Closure not proven`. |
...
| Harness Capability Closure | 执行框架能力闭合 | [Execution (V2 candidate)](./domain/execution/CONTEXT.md) | The A2 evidence matrix; its current result is `Closure not proven`. |
...
| Accepted Unsupported Dependency Risk | 已接受的不受支持依赖风险 | [Execution (V2 candidate)](./domain/execution/CONTEXT.md) | Exact owner resolution U2 for the selected Codex App Server closure subject under named controls, suspension conditions, and an exit plan. |
...
| Accepted Unsupported Dependency Risk | support claim / dependency selection / capability proof / Codex Capability Gap / Effect Approval / Run Authorization | The candidate term records one owner's bounded risk appetite. It does not change vendor maturity, select or install a dependency, prove closure or a gap, create domain authority, prove an Effect, or authorize implementation. |
...
- `已接受的不受支持依赖风险` preserves both halves of U2: the risk is explicitly accepted, and the dependency remains factually unsupported. Do not shorten it to `已接受风险` or translate it as vendor support, dependency approval, or production readiness.
