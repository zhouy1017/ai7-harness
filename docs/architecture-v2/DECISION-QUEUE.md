...
Status: **no current Owner decision blocks the accepted V2 `dev` baseline**
...
| Plugin admission exception | An identified need has no adequate AI7-owned or DSH-seam answer, and no candidate plugin meets the admission thresholds. | Whether to relax the thresholds for that case, fund an in-house implementation, or drop the capability. | The admission thresholds in root ADR 0042 hold, and no plugin below them enters the composition. |
