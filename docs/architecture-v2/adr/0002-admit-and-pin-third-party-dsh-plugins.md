...
This ADR is accepted inside the noncanonical V2 candidate. It records the owner's dependency-governance direction but does not edit canonical `main`, select a plugin, or authorize any search, download, installation, or implementation.

It is separate from [ADR 0001](./0001-dsh-first-deepseek-primary-architecture.md) because harness assignment and dependency governance are different trade-offs: one decides who owns the loop, the other decides how outside code enters the composition and how upstream change is prevented from reaching AI7 silently.
...
- A needed plugin that fails a threshold is not a defect to be argued around. Implement the behavior in AI7, use a DSH seam, or return the exception to the owner through the [Decision Queue](../DECISION-QUEUE.md).
...
This candidate decision derives from Clarification 0005 — `docs/architecture-exploration/clarifications/0005-dsh-first-model-routing-and-plugin-admission.md` at exact Commander commit `5693a5f444f0fb0daaa630444acc18932b0df391`.

It authorizes no GitHub search, plugin evaluation, plugin selection, download, installation, dependency change, source copy, prototype, product implementation, issue decomposition, pull request, push, merge, or release.
