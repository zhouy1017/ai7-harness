...
| DSH exposes the generic loop as an embeddable library inside the AI7 service process. | The composed runtime stays in-process with no TCP listener and no fourth authority. | Supervise a private local child process over stdio or a Windows named pipe behind the same boundary. Do not expose a network port and do not write an AI7 loop. |
...
| Windows filesystem confinement can combine restricted process rights with the AI7 Capability Facade. | The agent holds real permission only inside the Agent Data Root while semantic scope is checked again by AI7. | Tighten the process identity or move file access entirely behind AI7 capabilities. Never ask an editor to assess a generic escalation. |
...
| The pinned DSH subset is compatible with the shipped Node/Electron ABI and the Windows target. | One builder produces both Windows channels from one source. | Adjust the Electron/Node pairing or the subset. A named capability with no adequate Node implementation may enter as a bounded native module or sidecar under its own ADR. |
