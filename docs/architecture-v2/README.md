...
The current repository implementation remains only the bounded provider-free J-01 new-Book tracer: zero configured Providers, Agents, or Sessions; one atomic import; `r1`; a bounded first manuscript window; and durable journal acknowledgement. It has no executable artifact marketplace/lifecycle, Provider/egress selector, retrieval/Exact Fetch, covered analysis, Result Set, Enrollment, analysis metric, or AI7 Apply path. It still seeds an AI7 `WORKFLOW_PROFILE` and persists `workflow_profiles.definition_json`; that is a documented legacy/future migration gap, not evidence that the successor native-definition/AI7-projection seam exists. Those are future implementation obligations, not capabilities claimed by this document.
