...
2. **Keep the two restrictive v1 policy baselines.** Issue #22 establishes both active Policy Documents. Provider Processing v1 has exactly zero provider allow rules; External Export v1 makes only the exact accepted local-file Effect policy-eligible. Runtime enforcement, live-provider transmission, local-export implementation, and every network export remain unimplemented and require separate authority.
3. **Plan and deliver the first provider-free vertical result.** The bounded J-01 happy path begins from a fresh checkout and a public-synthetic DOCX, presents Review Before Import, atomically creates the Book, primary Manuscript, initial Manuscript Revision and import records, opens a bounded manuscript window, and confirms a durable Edit Journal. It uses the production-shaped Electron main, renderer, separate Node service, composed Harness/domain boundary, and one root bootstrap/build/launch/E2E command surface. The planning Issue first records the Supported Development Host matrix, exact Node version, and sole exact package-manager version. This tracer does not claim restart, ambiguous import outcome, every accepted branch, or full J-01.
4. **Continue only through later accepted vertical outcomes.** Task/Run/provider/model/export behavior, additional journeys, plugins, and packaging each require their own current authority and bounded Issue. A plugin remains need-based under root ADR 0042. Active Provider Processing v1 permits no live transmission, the External Export v1 eligibility rule grants no implementation authority, and no network export is admitted.
...
Start new business and technical stores. Do not import legacy Books, manuscripts, indexes, embeddings, memory, task/run/operation history, workflows, proposals, decisions, Effects, receipts, or UI state. The design-level legacy transfer allowlist remains limited to later user-initiated credential transfer into the new Protected Secret Store, separately reviewed mock-provider generators/fixtures, and explicitly selected test-only sample Books. None is used by Issue #20 or the current public-synthetic provider-free tracer; private or real manuscript material never becomes repository, hosted-CI, artifact, log, or distributable evidence.
