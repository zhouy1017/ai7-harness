...
2. **Land the two minimum policy baselines in a separately bounded Issue.** Provider Processing remains fail-closed unless an exact active rule permits a configured provider transmission; External Export permits only the exact accepted local-export boundary until later Owner authority exists. Neither policy is implemented in this documentation change.
...
4. **Continue only through later accepted vertical outcomes.** Task/Run/provider/model/export behavior, additional journeys, plugins, and packaging each require their own current authority and bounded Issue. A plugin remains need-based under root ADR 0042; no provider or export work proceeds before its active policy permits it.
