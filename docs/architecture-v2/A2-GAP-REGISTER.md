...
| --- | --- | --- | --- |
| **Unknown** | AI7 has not gathered the evidence. Missing documentation, an un-retrieved source path, or an unrun probe. | Naming exact next evidence. Nothing else. | 24 |
...
Twenty-four entries. Each names the exact next evidence that would move it. None is a finding about Codex's capability. Identifiers are not renumbered after retiring `UNK-A2-01` and `UNK-A2-20`.
...
| **UNK-A2-06** | `CC-16` | Whether shell and process execution can be made **absent** rather than sandbox-constrained. `thread/shellCommand` "runs outside the sandbox with full access"; `process/spawn` starts a process "outside sandbox" (`S-A2-02`, `S-A2-03`). Package inventory names a command runner but proves neither tool registration/reachability nor structural absence (`P-A2-03`). | Inspect the tool-registration surface in the exact source and prove a configuration with no shell or process tool registered. |
| **UNK-A2-07** | `CC-17` | Whether the `fs/*` family and `fileChange` tooling can be absent rather than root-restricted. Packaged executable names prove no filesystem capability boundary (`P-A2-03`). | Same tool-registration inspection; then attempt out-of-scope access and confirm structural refusal. |
...
| **UNK-A2-09** | `CC-19` | Whether coding-oriented features — `review/start` over `uncommittedChanges` / `baseBranch` / `commit`, `gitInfo`, `collaborationMode` presets, skills and hooks — can be excluded from an editorial Run. The packaged Code Mode host proves presence, not registration, reachability, or structural absence (`P-A2-03`). | Compose an editorial profile against the exact artifact and enumerate what remains reachable. |
...
| **UNK-A2-11** | `CC-22` | Windows sandbox enforcement strength, and whether `mode: "elevated"` setup surfaces a prompt to an editorial user. A packaged sandbox helper and cache-only trust observation prove neither enforcement nor elevation behavior (`P-A2-03`). | Measure Windows enforcement; determine whether unelevated setup suffices for AI7's profile. |
...
| **UNK-A2-14** | `CC-25` | Sidecar start, shutdown, process-tree, and orphan behavior under Electron. The exact native App Server entrypoint and its matching digest are now direct static evidence (`S-A2-14`, `P-A2-03`), but it was not executed. | Test clean start, shutdown, and process-tree teardown in both Windows channels. |
| **UNK-A2-15** | `CC-26` | Whether "Every thread in the same app-server process shares the selected Code Mode host connection" (`S-A2-02`) couples concurrent Runs or their mutable authority state, and how bounded queues and the 30-minute idle unload behave under AI7's parallel-Run requirement. Static package presence of a Code Mode host proves no concurrency isolation (`P-A2-03`). | Run ten concurrent Book executions with overlapping and disjoint readable scopes; measure authority isolation, interference, overload, and unload effects. |
...
| **UNK-A2-18** | `CC-32` | Whether any protocol compatibility contract exists. Neither `S-A2-02` nor `S-A2-03` documents versioning, compatibility, or breaking-change policy; no protocol schema was packaged, and generation, determinism, binary/schema equivalence, and drift rejection were not tested (`S-A2-14`, `P-A2-03`). U2 requires a gate but does not prove it. | Generate the stable schema twice from the exact artifact, record its fingerprint, compare it with the exact source output, and prove fail-closed drift handling during a rehearsed pin bump. |
| **UNK-A2-19** | `CC-35` | Whether the exact x64 sidecar packages and launches in both the zip-portable and NSIS channels. Safe static extraction proves neither channel integration nor launch (`P-A2-03`). | Package both channels against the exact artifact and run extract, first-run, launch, and removal. |
...
| **UNK-A2-22** | `CC-37` | Exact source `LICENSE` and `NOTICE` objects were retrieved and matched, while the complete package inventory carried neither (`S-A2-14`, `P-A2-03`). That absence is not artifact invalidity, noncompliance, or a legal conclusion; exact redistribution obligations, trademark treatment, and generated-notice reconciliation remain undone. | Review the exact source legal objects and dependency composition, determine the distribution obligations and trademark treatment, and reconcile the generated third-party notices for both channels without inferring package contents. |
...
## Disposition-transition history

These identifiers are retired from the active Unknown set and are preserved here without renumbering the survivors.

| ID | Row | Former Unknown | Transition |
| --- | --- | --- | --- |
| **UNK-A2-01** | `CC-01` | Which exact artifact is the closure subject. | Retired: Commander X2 (`S-A2-13`) selects stable `rust-v0.149.0` and its Windows x64 App Server package; `S-A2-14` plus `P-A2-03` establish the exact official identity and matching bytes. `CC-01` is Candidate, not Proven, because the rest of the closure matrix remains open and U2 imposes the Candidate ceiling. |
| **UNK-A2-20** | `CC-39` | Whether an exact, obtainable, immutable version exists with a tracked release channel. | Retired: the official stable release channel, selected immutable asset, successful official GET, and digest match are evidenced by `S-A2-13`, `S-A2-14`, and `P-A2-03`. `CC-39` is Candidate, not Proven: update cadence, pin-bump rehearsal, schema compatibility, and the complete upgrade contract remain untested. |
...
| --- | --- | --- | --- |
| **EXP-A2-01** | `CC-02` | `S-A2-02`: "The app-server command and WebSocket transport are experimental and aren't supported for production workloads." `S-A2-03`, at the pinned snapshot, scopes the same warning to WebSocket alone: "Websocket transport is currently experimental and unsupported. Do not rely on it for production workloads." | Support classification is the vendor's, not AI7's. U2 (`S-A2-12`) records Accepted Unsupported Dependency Risk for the exact subject under controls; it does not change what upstream supports, map the moving warning to exact release `0.149.0`, or raise this disposition. |
| **EXP-A2-02** | `CC-03` | The generic agent loop is reached only through the surface classified in `EXP-A2-01`. | Inherits `EXP-A2-01`. The loop itself is documented and is the surface `S-A2-01` explicitly recommends for AI7's shape; U2 remains a separate risk record. |
...
Several individually experimental features appear in the matrix without their own entry here, because a row can be Candidate while one of its mechanisms is experimental: `dynamicTools`, `thread/turns/list`, `thread/items/list`, `process/*`, `environment/info`, `thread/backgroundTerminals/*`, Beta permission profiles, and `plugin/*` (`S-A2-02`, `S-A2-03`). U2 does not accept those optional features by implication, and none may become a production premise without its own evidence and recorded compatibility decision.

## Gap claim

**None.** No entry in this register asserts that Codex cannot do something.

This is worth stating plainly because it is the register's most easily misread result. Twenty-four active Unknowns and two Experimentals look, at a glance, like a list of Codex shortcomings. They are not. Twenty-four of them are statements about **AI7's evidence**, and two are statements about **OpenAI's support policy**. Not one is a statement about what Codex is capable of.
...
If a future reader takes one thing from it: **absent evidence and proven limitation look similar in a table and are opposite in consequence.** An Unknown costs a probe or a source retrieval. A Verified Codex Capability Gap costs a secondary-development programme and can reopen a settled runtime choice. This register has twenty-four active entries of the first kind and none of the second.
