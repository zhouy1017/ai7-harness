...
/**
 * 设置 › 数据与存储's 导入数据库 (Issue #434, plan slice S86c; V2-UX-DSTO-017; ADR 0079 §1.3, §1.4): the preview of a database
 * file — what it holds, whether its Data Version fits this AI7, where it came from — the one choice, never preselected, of
 * what to do with it, the replacement waiting for AI7's next start, and what each replacement came to. Pure, so the unit suite
 * pins every line.
 */
...
/** The choices a previewed file offers; none is preselected (DSTO-017). */
...
export const DATABASE_REPLACEMENT_RECORDS = '替换记录';
export const DATABASE_REPLACEMENT_NO_RECORDS = '还没有替换过本机数据。';
...
/** The replacement waiting for AI7's next start, in three lines: what replaces what, the backup made, and when it completes. */
export function databasePendingLines(pending: Pick<DatabasePendingReplacementProjection, 'kind' | 'packageFileName' | 'backupFileName'>): readonly [string, string, string] {
...
/** 替换记录's summary: how many, and whether only the newest are listed. */
