...
export class AnalysisFeedbackLedger {
  ...
  *#signals(where: string, ...values: string[]): IterableIterator<StoredSignal> {
    ...
    let last: { chain: string; signalId: string; ordinal: number } | null = null;
    for (const row of rows) {
      ...
      last = { chain, signalId: String(row.signal_id), ordinal };
      yield {
  ...
  #latest(where: string, ...values: string[]): StoredSignal | null {
    let latest: StoredSignal | null = null;
    for (const signal of this.#signals(where, ...values)) latest = signal;
    return latest;
  }
  ...
  record(bookId: string, revision: AnalysisFeedbackRevision, items: ReadonlyArray<AnalysisFeedbackItem>, input: RecordAnalysisFeedbackInput): void {
    ...
    const latest = this.#latest('revision_id = ? AND item_key = ?', revision.revisionId, item.itemKey);
  ...
  metric(bookId: string): AnalysisQualityMetricProjection {
    // Validate every successor before using SQL's latest-per-item selection. The database owns lifetime sorting/state.
    for (const _signal of this.#signals('book_id = ?', bookId)) { /* complete chain validation */ }
    const counts = { judged: 0, accurate: 0, inaccurate: 0, incomplete: 0 };
    const byDimension = ANALYSIS_FEEDBACK_DIMENSIONS.map((dimension) => ({ dimension, ...counts }));
    const latest = this.#db.prepare(`SELECT signal_id, sha256, judgment, item_key FROM analysis_feedback_signals AS signal
      WHERE book_id = ? AND NOT EXISTS (SELECT 1 FROM analysis_feedback_signals AS successor
        WHERE successor.book_id = signal.book_id AND successor.revision_id = signal.revision_id AND successor.item_key = signal.item_key AND successor.ordinal > signal.ordinal)
      ORDER BY signal_id`).iterate(bookId) as IterableIterator<SqlRow>;
    // Byte-identical to the existing canonical object: signals is its last sorted key, entries remain signal-id ordered.
    const hash = createHash('sha256');
    hash.update(canonicalJson({ bookId, definition: ANALYSIS_QUALITY_METRIC_DEFINITION, schema: LINEAGE_SCHEMA }).slice(0, -1));
    hash.update(',"signals":[');
    let first = true;
    for (const row of latest) {
      const judgment = String(row.judgment) as AnalysisFeedbackJudgment;
      const dimension = byDimension.find((entry) => entry.dimension === dimensionOf(String(row.item_key)))!;
      counts.judged += 1;
      counts[judgment] += 1;
      dimension.judged += 1;
      dimension[judgment] += 1;
      if (!first) hash.update(',');
      hash.update(canonicalJson({ signalId: String(row.signal_id), sha256: String(row.sha256) }));
      first = false;
    }
    hash.update(']}');
    return {
  ...
  projection(bookId: string, revision: AnalysisFeedbackRevision, items: ReadonlyArray<AnalysisFeedbackItem>): AnalysisFeedbackProjection {
    return {
      ...
      items: items.map((item): AnalysisFeedbackItemProjection => {
        const last = this.#latest('book_id = ? AND revision_id = ? AND item_key = ?', bookId, revision.revisionId, item.itemKey);
        return {
          itemKey: item.itemKey,
          dimension: item.dimension,
          index: item.index,
