...
export class BaselineAnalysisStore {
  ...
  inspect(bookId: string, progress: ProgressReader = () => null, revisionId: string | null = null): AnalysisProjection {
    ...
    return this.#asProjection({
      ...
      inspectedRevision,
      actions: {
        // A baseline analysis whose Run ended before it kept anything — cancelled, failed, interrupted or blocked —
        // leaves the Book with no revision: it can be started again, as a Book never analysed can (Issue #422, S76c).
  ...
  loadExecutionPlan(runRecordId: string): ExecutionPlanFacts {
    ...
    // An adaptation moved into 先问你 (Issue #422, S76d) is in the split's own list, never the adaptable one.
    const asked = PLAN_ADAPTATION_CLASSES.filter((adaptationClass) => (editorEdits.askFirstAdaptations ?? []).includes(adaptationClass));
    const allowed = PLAN_ADAPTATION_CLASSES.filter((adaptationClass) => !editorEdits.disallowedAdaptations.includes(adaptationClass) && !asked.includes(adaptationClass));
