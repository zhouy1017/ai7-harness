 ...
 * Adaptation, the safe retry, can be withdrawn. Rewriting, adding or reordering a step, and the reference the update
 * stands on, have nothing in the Run to change, so they are not offered.
 */
 ...
 * requests that mean the same edit are the same record. `null` when anything in it is not an editable item.
 */
export function canonicalPlanEdits(input: { removedSteps: unknown; disallowedAdaptations: unknown }): PlanEdits | null {
  const { removedSteps, disallowedAdaptations } = input;
  if (!Array.isArray(removedSteps) || !Array.isArray(disallowedAdaptations)) return null;
  ...
  if (new Set(removedSteps).size !== removedSteps.length || new Set(disallowedAdaptations).size !== disallowedAdaptations.length) return null;
  return {
...
export function planEditsOf(executionPlan: unknown): PlanEdits {
  ...
  const edits = record !== null && typeof record === 'object' ? canonicalPlanEdits(record as { removedSteps: unknown; disallowedAdaptations: unknown }) : null;
...
export function planEditsAreEmpty(edits: PlanEdits): boolean {
  return edits.removedSteps.length === 0 && edits.disallowedAdaptations.length === 0;
}
...
/** Whether the Run this plan binds may make the safe retry. */
export function safeRetryAllowed(edits: PlanEdits): boolean {
  return !edits.disallowedAdaptations.includes('safe-retry');
}
...
export function planEditDiff(prior: PlanEdits, next: PlanEdits): PlanRevisionDiffEntryProjection[] {
  ...
  for (const adaptation of PLAN_EDITABLE_ADAPTATIONS) {
    const before = prior.disallowedAdaptations.includes(adaptation);
    const after = next.disallowedAdaptations.includes(adaptation);
    if (before !== after) {
      entries.push({
        field: `adaptations.${adaptation}`,
        label: PLAN_EDIT_ADAPTATION_LABELS[adaptation],
        prior: before ? '不允许' : '允许',
        proposed: after ? '不允许' : '允许',
        materiality: 'edited',
      });
    }
  }
