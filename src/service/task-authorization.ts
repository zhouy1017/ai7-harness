 ...
 * nothing is backfilled. No existing row changes. This is the terminal version.
 */
...
export function validateTaskAuthorizationSchema(db: DatabaseSync): void {
  ...
  requireTask(version === PRODUCTION_DOCUMENT_ORIGIN_SCHEMA_VERSION, 'SCHEMA_UNSUPPORTED', '数据库版本不受支持。');
...
function migrateAnalysisLedgerToRevision17(db: DatabaseSync, from: typeof J04_BASELINE_ANALYSIS_SCHEMA_VERSION | typeof SUCCESSIVE_TASK_SCHEMA_VERSION): void {
  ...
  try {
    try {
      ...
      db.exec(`PRAGMA user_version = ${PRODUCTION_DOCUMENT_ORIGIN_SCHEMA_VERSION}`);
      requireTask(db.prepare('PRAGMA foreign_key_check').all().length === 0, 'SCHEMA_MIGRATION_FAILED', '分析任务账本迁移后引用校验失败。');
      validateTaskAuthorizationSchema(db);
      db.exec('COMMIT');
    } catch (error) {
...
function advanceToTerminalRevision(db: DatabaseSync): void {
  migrateInTransaction(db, `PRAGMA user_version = ${PRODUCTION_DOCUMENT_ORIGIN_SCHEMA_VERSION};`, 'Terminal version');
}
...
export function initializeTaskAuthorizationSchema(db: DatabaseSync): void {
  ...
  if (version === PRODUCTION_DOCUMENT_ORIGIN_SCHEMA_VERSION) return validateTaskAuthorizationSchema(db);
  ...
  if (version === EDITORIAL_REVIEW_SCHEMA_VERSION || version === PUBLICATION_VERSION_SCHEMA_VERSION ||
    ...
    // four they move, and revisions 36 to 42 add none, so the ledger a revision-24 to revision-41 store carries is
