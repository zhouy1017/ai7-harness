...
export class DataVersionLedger {
  ...
  *history(): IterableIterator<StoredVersion> {
  ...
  /** Validate the whole ledger, retaining only the current record, latest software transition and newest twenty. */
  standing(): { latest: StoredVersion | null; recent: StoredVersion[]; update: ReturnType<typeof latestSoftwareUpdate>; count: number } {
    ...
    return { latest, recent: recent.reverse(), update, count: latest?.ordinal ?? 0 };
  }
