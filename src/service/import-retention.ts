...
/** One row of the block-to-source-paragraph mapping a committed Revision carries. */
export interface ManuscriptBlockSource {
  blockId: string;
  sourcePart: 'body' | 'text-box';
  sourceParagraphIndex: number;
  boxOrdinal: number | null;
  boxParagraphOrdinal: number | null;
  sourceParagraphDigest: string;
}

/** Record the mapping of a committed Revision, in the caller's transaction. */
export function recordManuscriptBlockSources(
  db: DatabaseSync,
  revisionId: string,
  sourceVersionId: string,
  sources: Iterable<ManuscriptBlockSource>,
  createdAt: string,
): number {
  ...
  for (const source of sources) {
    insert.run(
      revisionId, source.blockId, sourceVersionId, source.sourcePart, source.sourceParagraphIndex,
      source.boxOrdinal, source.boxParagraphOrdinal, source.sourceParagraphDigest, createdAt,
    );
    written += 1;
  }
  return written;
...
/** How many blocks of a Revision the mapping covers. */
export function manuscriptBlockSourceCount(db: DatabaseSync, revisionId: string): number {
  return integer((db.prepare('SELECT count(*) total FROM manuscript_block_sources WHERE revision_id = ?').get(revisionId) as SqlRow).total);
}
