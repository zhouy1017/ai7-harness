...
export const REPORT_EXPORT_FORMATS: ReadonlyArray<ManuscriptExportFormatProjection> = [
  { format: 'docx', label: 'DOCX', available: true, note: '主要可编辑格式：可在 Word 中继续修改，概览表写成表格。', fallback: false },
  { format: 'pdf', label: 'PDF', available: true, note: '可选 · 固定版式，适合阅读与打印；不能继续编辑。', fallback: false },
  { format: 'markdown', label: 'Markdown（备用格式）', available: true, note: '备用格式 · 只写出文字、标题层级与表格，用于迁移或留底。', fallback: true },
];
