...
| **Milestone Version** | 里程碑版本 | A user-designated exact Manuscript Revision with a stated next-use purpose but no export or release authority; only the primary Manuscript has milestones (ADR 0077). | 最新草稿、签发记录、发稿版本、生产文档版本 |
| **Delivery Package** | 交付包 | Retained only for the manifest rules the Book Delivery Package inherits; no per-deliverable package exists (ADR 0077). | 导出文件、本地导出准备、路径绑定压缩包、交付或发布证明、交付记录 |
| **Publication Version** | 发稿版本 | An append-only exact designation over one Milestone Version of the primary Manuscript (发稿), linked to a separate Public Release Permission and proving no export, sending, delivery, or publication. | 最新草稿、已发布成果、本地导出、公开发布许可 |
| **Maintenance Case** | 维护事项 | A stable post-designation matter permanently bound to one exact Publication Version and its Manuscript Revision under one Maintenance Classification. | 工作阶段、自由备注、外部召回请求 |
| **Production Document** | 生产文档 | An Editorial Deliverable other than the primary Manuscript — a house-configured type such as 新闻稿, 宣传文章, 评论文章, 发布会材料 or 营销要点 — with its own versions, Workflow Instance, gates and Delivery Records (ADR 0077). | 稿件、交付包、编辑工作资料 |
| **Delivery Record** | 交付记录 | One handover of one exact Production Document version to a named recipient, with actor, time, note and the linked local export receipt; independent of 发稿 and repeated by a new record after a later version (ADR 0077). | 发稿版本、交付包、受控动作批准、外部送达证明 |
| **Book Delivery Package** | 图书交付包 | The Book-level frozen bundle of the Manuscript's Publication Version, the latest delivered version of every Production Document not marked 本书不做 and the work records; versioned on any change, with a separate export history (ADR 0077). | 单份文档的包、导出文件、发稿、交付 |
...
- A **Production Document** is handed over through a **Delivery Record** of one exact version; the **Book Delivery Package** binds the Manuscript's Publication Version and every delivered document and stays independent of format and destination (ADR 0077). Each concrete file instead has its own **Local Export Preparation**, exact approval-before-commit, and per-file receipt; native rename/cancel/replace presentation grants no authority and a local receipt proves no sending or publication.
- A **Maintenance Case** never changes its target or **Maintenance Classification**. **Maintenance Case Revisions** append history; correction/reissue may create new Manuscript Revisions, and supersession/reissue requires a separately manual Publication Version designation, while withdrawal/archive claim no external recall or takedown.
...
- “Version / 版本” must be qualified as **源材料版本**, **稿件修订版**, **编辑交付成果修订版**, **里程碑版本**, **发稿版本**, **生产文档版本**, **图书交付包版本**, **数据版本**, or another exact accepted record type.
