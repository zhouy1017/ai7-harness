...
async function main() {
  ...
  try {
    ...
    await assertRenderer(renderer, `(() => { const track = document.querySelector('.rail-track'); const markers = Array.from(track.querySelectorAll('.rail-marker')); const lefts = new Set(markers.map((marker) => Math.round(marker.getBoundingClientRect().left))); const entries = Array.from(document.querySelectorAll('.edge-entries button')).map((entry) => entry.textContent + ':' + entry.disabled); return lefts.size === 3 && markers.every((marker) => marker.getAttribute('aria-label').includes('处')) && track.dataset.railAnalysed === 'false' && track.querySelectorAll('.rail-gap').length === 0 && entries.join('|') === '导航:false|分析:false|任务:false' && document.querySelector('#manuscript-navigation-panel').hidden === true; })()`, 'rail-lanes-and-edge-entries');
