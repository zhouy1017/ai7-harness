...
async function main() {
  ...
  try {
    ...
    await assertRenderer(renderer, `(() => { const card=document.querySelector('.task-authorization-card'); const button=card?.querySelector('[data-task-authorization-action="authorize-no-dispatch"]'); const rows=(selector)=>{ const result={}; for(const term of card?.querySelectorAll(selector)??[]) result[term.textContent??'']=term.nextElementSibling?.textContent??''; return result; }; const preview=rows(':scope > dl.task-authorization-facts > dt'); const exact=rows(':scope > details.technical-details > dl > dt'); return card?.textContent.includes('仅血缘证据，不属于可读范围') && card.textContent.includes(${JSON.stringify(imported.bookId)}) && card.textContent.includes(${JSON.stringify(prepared.manuscriptPin.manuscriptId)}) && card.textContent.includes(${JSON.stringify(NATIVE_CARRIER_DIGEST)}) && !card.textContent.includes('空（无）') && preview['未声明']==='Capability、Approved Fallback Chain、Effect' && preview['Capability']===undefined && preview['Approved Fallback Chain']===undefined && preview['Effect']===undefined && preview['Provider Binding']===${JSON.stringify(`${prepared.providerResolutionPlan.providerId} · ${prepared.providerResolutionPlan.modelId}`)} && preview['Credential Reference']===${JSON.stringify(`readiness ${prepared.providerResolutionPlan.credentialReadiness}`)} && exact['Provider Binding（适配器与契约）']===${JSON.stringify(`adapter r${prepared.providerResolutionPlan.adapterRevision} · config r${prepared.providerResolutionPlan.configurationRevision}`)} && exact['Credential Reference']===${JSON.stringify(prepared.providerResolutionPlan.credentialReference)} && card.textContent.includes('development-ci · v1 · 拒绝 · 0 次实时传输') && button?.textContent==='记录本次运行授权（不派发）'; })()`, 'prepared-preview');
