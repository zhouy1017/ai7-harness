...
// The legacy binary Word document the converted `.doc` intake can only be shown on. ADR 0079 §5
// narrowed the repository to exact `sample1` and no generator for the format exists, so this is
// local-only test material (`e2e/controller.mjs`): present, the scenario runs; absent — which is
// every hosted occurrence — the Journey discloses the skip and continues. It is read in place.
const LOCAL_DOC_PATH = localManuscriptPath(LOCAL_ONLY_DOC);
const LOCAL_DOC_AVAILABLE = localManuscriptAvailable(LOCAL_ONLY_DOC);
...
async function runJourney(
  ...
  await assertRenderer(
    renderer,
    `typeof globalThis.process === 'undefined' && typeof globalThis.require === 'undefined' && Object.keys(window.ai7).sort().join(',') === 'abandonImportDraft,acceptReimportDegradation,acknowledgeImportCompletion,addLibraryMaterial,answerBaselineAnalysisClarification,appendMaintenanceCaseRevision,applyChangeSuggestion,applyChangeSuggestionBatch,approveBookDeliveryPackageExport,approveDatabaseExport,approveManuscriptExport,authorizeBaselineAnalysis,authorizeReviewRun,authorizeTaskAuthorization,cancelBaselineAnalysisRun,cancelDatabaseExport,cancelDatabaseReplacement,cancelServiceJob,cancelWaitingBaselineAnalysis,changeSeriesMembership,chooseBookDeliveryPackageExportFolder,chooseDatabaseExportDestination,chooseDatabaseImportFile,chooseManuscriptExportDestination,commitBookCreation,commitManuscriptReimport,commitNewBookImport,commitReplacement,commitSourceImport,continueImportDraft,continueReviewRun,createEditorialMark,createProductionDocument,createSeries,deactivateDefaultExecutionRule,decideLearningMaterial,decideLibraryMaterial,decideProductionDocumentType,deferRecovery,designatePublicationVersion,dismissReplacementPreview,editBaselineAnalysisPlan,editSeriesKnowledgeCandidate,enableEditorialWorkspaceProfile,flushJournalEdit,freezeReplacement,generateReviewReport,getBookOverview,getBookWorkbenchRoute,getEditorialMarkCard,getHistoricalRevision,getImportStartup,getManuscriptApplyOutcome,getManuscriptRail,getManuscriptWindow,getManuscriptWindowAt,getModelServiceSettings,getOutline,getProductDataLocation,getRecoveryComparison,getReimportLineageSourceVersionPage,getReimportMappingPage,getSearchResults,getStartup,importReviewGuidelineVersion,inspectAnalysisFeedback,inspectBaselineAnalysis,inspectBookDeliveryPackage,inspectBookSeries,inspectBookTasks,inspectDataVersion,inspectDatabaseExports,inspectDatabaseReplacements,inspectDefaultExecutionRules,inspectDeliverables,inspectEditorialWorkspaceProfile,inspectEvaluation,inspectEvaluationCalibration,inspectEvaluationProfiles,inspectExemplars,inspectFeedbackHistory,inspectForegroundExecutionBoundary,inspectGlobalAttention,inspectKnowledgeProcedures,inspectLearningMaterial,inspectLearningMaterials,inspectLibraryMaterial,inspectLibraryMaterials,inspectMaintenanceCase,inspectProductionDocuments,inspectProposalConflict,inspectReviewFindingOfMark,inspectReviewGuidelines,inspectReviewWorkspace,inspectScheduledBackups,inspectSeries,inspectSeriesCandidates,inspectSeriesHistory,inspectSeriesKnowledgeCandidates,inspectSeriesKnowledgeItems,inspectSeriesKnowledgeReview,inspectSeriesKnowledgeRevisions,inspectSeriesList,inspectSeriesMembers,inspectTaskAuthorization,inspectTaskPlan,installEditorialWorkspaceProfile,leaveBookWorkbench,listBooks,listMaintenanceCases,listPriorWork,openBookWorkbench,pauseBaselineAnalysisRun,platform,pollServiceJob,prepareBaselineAnalysis,prepareBookCreation,prepareBookDeliveryPackage,prepareDatabaseReplacement,prepareManuscriptReimport,prepareNewBookReview,prepareReplacement,prepareReviewRun,prepareSourceImportReview,prepareTaskAuthorization,previewLibraryMaterial,previewReviewGuidelineVersion,previewSeriesMembershipChange,promoteSeriesKnowledge,proposeSeriesKnowledge,quickStartBaselineAnalysis,quitApplication,recordAnalysisFeedback,recordChangeSuggestionDecision,recordMaintenanceCase,recordManuscriptEntryPosition,recordProductionDocumentDelivery,recordProposalDecisionFeedback,recordProposalDecisionReason,recordPublicationActuals,recordReviewFindingDisposition,redoManuscript,removeModelServiceCredential,reselectImportDraft,resolveProposalConflict,resolveReimportMapping,restoreRecovery,resumeBaselineAnalysisRun,revealManuscriptExport,revealProductDataLocation,reverseAppliedChangeSuggestion,reviewBookDeliveryPackageExport,reviewManuscriptExport,rollBackDatabaseReplacement,runEditorClipboardCommand,runReconnectPreflight,saveEvaluation,saveMaintenanceErrata,saveMilestone,saveModelServiceCredential,saveProductionDocumentVersion,saveProposalConflictDraft,selectAndStageManuscript,setDefaultExecutionRule,setEvaluationPreferences,setScheduledBackup,startBaselineAnalysisWhenOnline,startEvaluation,startReplacementCommit,startSearch,transitionProductionDocumentPhase,undoManuscript,updateBookPeople,updateEditorialMark,viewRecoveryCandidate'`,
    'renderer-isolation',
  );
  await assertRenderer(
    renderer,
...
async function main() {
  ...
  try {
    ...
    // 0079 §5) instead of authored sentences: twelve paragraphs of the one admitted source,
    // extracted at run time and never committed. `admittedBlocks` already excludes any paragraph
    // that normalizes to nothing, so the trim-and-drop below is a defensive check over an admitted
    ...
    // Only when the local-only material is present: identity is proven before the file is ever
    // handed to the product, exactly as it was while the file was admitted.
    if (LOCAL_DOC_AVAILABLE) {
      const localDocInfo = await lstat(LOCAL_DOC_PATH);
      requireJourney(
        localDocInfo.isFile() &&
          !localDocInfo.isSymbolicLink() &&
          localDocInfo.size === LOCAL_DOC_BYTES &&
          (await realpath(LOCAL_DOC_PATH)) === LOCAL_DOC_PATH &&
          (await digestFile(LOCAL_DOC_PATH)) === LOCAL_DOC_SHA256,
        'local-doc-identity',
      );
    }
    ...
    const launchProduct = async ({ dataRoot, pickerPath, importControl, launchScenario }) => {
      ...
      requireJourney(
        isAbsolute(dataRoot) &&
          (!pickerPath ||
            (isAbsolute(pickerPath) &&
              (pickerPath === docx ||
                (LOCAL_DOC_AVAILABLE && pickerPath === LOCAL_DOC_PATH) ||
                pathIsInside(runRoot, pickerPath)))) &&
    ...
    requireJourney(
      textManuscriptRecord?.['格式'] === 'TXT' &&
        ...
        textManuscriptRecord?.['解析器'] === 'ai7-docx-fflate-saxes/3' &&
    ...
    // could not carry across is dropped content, not characters kept as the author typed them. Only
    // a real legacy document can carry that route, and ADR 0079 §5 left the one such document
    // local-only, so the scenario runs where the developer has it and is disclosed as skipped
    // wherever it is absent. Every other J-01 scenario is unaffected either way.
    if (LOCAL_DOC_AVAILABLE) {
      ...
      await assertRenderer(
        renderer,
        `document.querySelector('[data-source-format]')?.dataset.sourceFormat === 'DOC' && document.querySelector('[data-source-sha256]')?.textContent === ${JSON.stringify(LOCAL_DOC_SHA256)} && document.querySelector('[data-source-bytes]')?.textContent === ${JSON.stringify(String(LOCAL_DOC_BYTES))}`,
      ...
      await assertRenderer(
        renderer,
        `document.body.textContent.includes('${LOCAL_DOC_BLOCKS} 个可编辑内容块')`,
      ...
      await assertRenderer(
        renderer,
        `(() => { const note = document.querySelector('[data-screen="review"] [data-import-conversion-note]'); return note?.dataset.importConversionNote === 'DOC' && document.querySelector('[data-screen="review"] [data-source-sha256]')?.textContent === ${JSON.stringify(LOCAL_DOC_SHA256)}; })()`,
      ...
      requireJourney(
        docManuscriptRecord?.['格式'] === 'DOC' &&
          ...
          docManuscriptRecord?.['解析器'] === 'ai7-docx-fflate-saxes/3' &&
      ...
      discloseJourneySkip('J-01', 'doc-manuscript-local-only-absent');
    }
