...
/**
 * The kind and heading level of each paragraph of the same excerpt, as the product classifies its style (Issue #500,
 * S64b): what a format that writes heading levels — the Markdown 备用格式 — writes for it.
 */
export async function admittedParagraphShapes({ source, startBlock, blocks }) {
  ...
  return available.slice(startBlock - 1, lastBlock).map((block) => block.style === 'Title'
    ? { kind: 'title', level: 1 }
    : block.style?.startsWith('Heading') ? { kind: 'heading', level: Number(block.style.slice('Heading'.length)) } : { kind: 'paragraph', level: null });
}
