...
function plan(overrides: Partial<TaskPlanProjection> = {}): TaskPlanProjection {
  return {
    ...
    edit: { editable: true, reason: null, lastEdit: null, planEnvelopeDigest: 'a'.repeat(64), budget: { ceiling: 'unset', settable: true, reason: null } },
    drift: null,
    technical: [],
...
describe('the authorization bar (S74a)', () => {
  ...
  it('says J-03\'s Task is only recorded, and a plan without a route is blocked before dispatch, beside the same start (ADR 0055)', () => {
    const recordOnly = taskBarView(barOf({ readiness: 'record-only' }, { kind: 'fixed-task', planVersion: null, edit: { editable: false, reason: null, lastEdit: null, planEnvelopeDigest: null, budget: null } }));
...
describe('the editable plan (S73)', () => {
  const edit = (overrides: Partial<TaskPlanProjection['edit']> = {}): TaskPlanProjection['edit'] =>
    ({ editable: true, reason: null, lastEdit: null, planEnvelopeDigest: 'a'.repeat(64), budget: { ceiling: 'unset', settable: true, reason: null }, ...overrides });
