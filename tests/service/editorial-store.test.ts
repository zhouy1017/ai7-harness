...
function downgradeToRevision20(databasePath: string): void {
  ...
  try {
    ...
    database.exec(`BEGIN IMMEDIATE;
      ${[...CLARIFICATION_RELATIONS_DROP_ORDER, ...RUN_CHECKPOINT_RELATIONS_DROP_ORDER, ...DEFAULT_EXECUTION_RULE_RELATIONS_DROP_ORDER, ...EXPORT_LEDGER_RELATIONS_DROP_ORDER, ...IMPORTED_MARK_RELATIONS_DROP_ORDER, ...IMPORT_RETENTION_RELATIONS_DROP_ORDER].map((relation) => `DROP TABLE ${relation};`).join('\n      ')}
  ...
  it('migrates a populated revision-20 store forward, adding eleven empty relations, reshaping only the three kind-coupled analysis relations, and moving nothing else', async () => {
    ...
    try {
      expect((after.prepare('PRAGMA user_version').get() as { user_version: number }).user_version)
        .toBe(CLARIFICATION_SCHEMA_VERSION);
      ...
      const added = [
        ...
        ...CLARIFICATION_RELATIONS_DROP_ORDER, ...RUN_CHECKPOINT_RELATIONS_DROP_ORDER, ...DEFAULT_EXECUTION_RULE_RELATIONS_DROP_ORDER,
