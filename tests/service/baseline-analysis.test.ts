...
import {
  ...
  CLARIFICATION_SCHEMA_VERSION, PRODUCTION_DOCUMENT_WORKFLOW_SCHEMA_VERSION,
 ...
 * downgraded store is not the shape it claims. So are revision 27's import-retention relations and
...
describe('baseline manuscript analysis over the real store on exact sample1', () => {
  it('executes the authorized Run through the real path and reaches one immutable Result Set Revision', async () => {
    ...
    try {
      expect((database.prepare('PRAGMA user_version').get() as { user_version: number }).user_version).toBe(PRODUCTION_DOCUMENT_WORKFLOW_SCHEMA_VERSION);
  ...
  it('migrates a revision-14 store forward without rewriting any J-03 row', async () => {
    ...
    try {
      expect((verify.prepare('PRAGMA user_version').get() as { user_version: number }).user_version).toBe(PRODUCTION_DOCUMENT_WORKFLOW_SCHEMA_VERSION);
      for (const table of Object.keys(ANALYSIS_LEDGER_SCHEMA_SQL)) {
