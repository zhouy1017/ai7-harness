...
/**
 * Revision 21's entry-position relation and revision 22's editorial-mark relations are simply not
 * there below them, so a store taken back to any earlier revision loses them again — otherwise the
 * downgraded store is not the shape it claims.
 */
function dropEntryPositionRelation(database: DatabaseSync): void {
  for (const relation of ['proposal_decision_reasons', 'proposal_item_decisions', 'proposal_change_items', 'editorial_mark_replies', 'editorial_marks']) {
...
describe('baseline manuscript analysis over the real store on exact sample1', () => {
  it('executes the authorized Run through the real path and reaches one immutable Result Set Revision', async () => {
    ...
    try {
      expect((database.prepare('PRAGMA user_version').get() as { user_version: number }).user_version).toBe(EDITORIAL_MARK_SCHEMA_VERSION);
  ...
  it('migrates a revision-14 store forward without rewriting any J-03 row', async () => {
    ...
    try {
      expect((verify.prepare('PRAGMA user_version').get() as { user_version: number }).user_version).toBe(EDITORIAL_MARK_SCHEMA_VERSION);
      for (const table of Object.keys(ANALYSIS_LEDGER_SCHEMA_SQL)) {
