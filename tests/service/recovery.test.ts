...
async function importComposedBook(store: EditorialStore): Promise<{
  ...
  // What is recovered here is edited manuscript text, so the manuscript is composed from exact
  // `sample1`, the one admitted Public SampleBook, rather than generated. Six blocks are enough: the
  // subject is the journal, not size.
