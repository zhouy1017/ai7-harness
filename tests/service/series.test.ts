...
import { EVALUATION_CALIBRATION_SCHEMA_VERSION, STORE_VERSION_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
...
describe('书系 over the real store', () => {
  ...
  it('adds revision 52 to a revision-51 store with nothing else moved', async () => {
    ...
    try {
      plant.exec(`DROP TABLE store_versions; DROP TABLE series_knowledge_promotions; DROP TABLE series_knowledge_revisions; DROP TABLE series_knowledge_candidates; DROP TABLE series_knowledge_items; DROP TABLE series_membership_changes; DROP TABLE series; PRAGMA user_version = ${EVALUATION_CALIBRATION_SCHEMA_VERSION};`);
    ...
    try {
      ...
      expect(after.filter((entry) => !/^(series|store_versions)/u.test(entry.name))).toEqual(before!);
