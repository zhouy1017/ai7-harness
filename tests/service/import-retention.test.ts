...
import { IMPORT_RETENTION_SCHEMA_VERSION, PROPOSAL_CONFLICT_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
...
describe('schema revision 27 over the real store', () => {
  it('migrates a revision-26 store holding a legacy eight-row review, with every fidelity row byte for byte', async () => {
    ...
    withDatabase(true, (database) => {
      ...
      for (const relation of IMPORT_RETENTION_RELATIONS_DROP_ORDER) {
