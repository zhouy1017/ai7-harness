...
import { SERIES_KNOWLEDGE_SCHEMA_VERSION, DATABASE_EXPORT_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
...
describe('数据版本 over the real store', () => {
  it('records the versions that open the store, and says a software update kept the Data Version', async () => {
    ...
    expect(first).toEqual({
      ...
      update: null,
      history: [{ softwareVersion: version, dataVersion: 1, schemaRevision: DATABASE_EXPORT_SCHEMA_VERSION, recordedAt: first.history[0]!.recordedAt }],
    ...
    try {
      ...
      const earlier = canonicalRecord({
        ...
        schemaRevision: DATABASE_EXPORT_SCHEMA_VERSION, supersedes: null, recordedAt,
      });
      database.prepare(`INSERT INTO store_versions(record_id, ordinal, software_version, data_version, schema_revision, supersedes_record_id, recorded_at, canonical_json, sha256)
        VALUES (?, 1, '0.0.9', 1, ?, NULL, ?, ?, ?)`).run('00000000-0000-4000-8000-000000000001', DATABASE_EXPORT_SCHEMA_VERSION, recordedAt, earlier.json, earlier.digest);
    } finally {
      database.close();
    }
  ...
  it('keeps the records as a verified chain appended only on a change, and lists the newest twenty', async () => {
    ...
    const plant = async (from: number, to: number): Promise<void> => {
      ...
      try {
        ...
        for (let patch = from; patch <= to; patch += 1) ledger.recordOpen({ softwareVersion: `0.0.${patch}`, dataVersion: 1, schemaRevision: DATABASE_EXPORT_SCHEMA_VERSION });
      } finally {
        store.close();
      }
    };
  ...
  it('adds revision 54 to a revision-53 store, recording the versions that open it from then on', async () => {
    ...
    try {
      plant.exec(`DROP TABLE database_export_receipts; DROP TABLE database_export_approvals; DROP TABLE database_export_preparations; DROP TABLE store_versions; PRAGMA user_version = ${SERIES_KNOWLEDGE_SCHEMA_VERSION};`);
    } finally {
      plant.close();
    }
    const migrated = await reopened((store) => store.inspectDataVersion());
    expect([migrated.schemaRevision, migrated.history.length, migrated.update]).toEqual([DATABASE_EXPORT_SCHEMA_VERSION, 1, null]);
