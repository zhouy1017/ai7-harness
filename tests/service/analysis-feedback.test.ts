...
import { DECISION_FEEDBACK_SCHEMA_VERSION, EVALUATION_RECORD_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
import {
...
describe('②A 分析反馈 over the real store', () => {
  ...
  it('adds revision 48 to a revision-47 store with nothing else moved', async () => {
    ...
    try {
      plant.exec(`DROP TABLE proposal_decision_feedback; DROP TABLE analysis_feedback_signals; PRAGMA user_version = ${EVALUATION_RECORD_SCHEMA_VERSION};`);
    } finally {
      plant.close();
    }
