...
import { EVALUATION_RECORD_SCHEMA_VERSION, LIBRARY_MATERIAL_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
...
describe('②C 评估 over the real store', () => {
  ...
  it('adds revision 47 to a revision-46 store with nothing else moved', async () => {
    ...
    try {
      plant.exec(`DROP TABLE evaluation_record_entries; DROP TABLE evaluation_records; PRAGMA user_version = ${LIBRARY_MATERIAL_SCHEMA_VERSION};`);
    } finally {
      plant.close();
    }
