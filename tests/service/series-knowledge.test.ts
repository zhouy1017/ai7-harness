...
import { DATABASE_REPLACEMENT_SCHEMA_VERSION, SERIES_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
...
describe('书系知识 over the real store', () => {
  ...
  it('adds revision 53 to a revision-52 store with nothing else moved', async () => {
    ...
    try {
      ...
      expect(after.filter((entry) => !/^(series_knowledge|store_versions|database_export_|backup_preferences|scheduled_backup|database_replacements)/u.test(entry.name))).toEqual(before!);
