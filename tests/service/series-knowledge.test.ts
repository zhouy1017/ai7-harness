...
import { DATABASE_EXPORT_SCHEMA_VERSION, SERIES_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
...
describe('书系知识 over the real store', () => {
  ...
  it('adds revision 53 to a revision-52 store with nothing else moved', async () => {
    ...
    try {
      plant.exec(`DROP TABLE database_export_receipts; DROP TABLE database_export_approvals; DROP TABLE database_export_preparations; DROP TABLE store_versions; ${TABLES.slice().reverse().map((table) => `DROP TABLE ${table};`).join(' ')} PRAGMA user_version = ${SERIES_SCHEMA_VERSION};`);
    ...
    try {
      ...
      expect(after.filter((entry) => !/^(series_knowledge|store_versions|database_export_)/u.test(entry.name))).toEqual(before!);
