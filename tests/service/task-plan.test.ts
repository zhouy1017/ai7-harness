...
describe('the Task Drawer plan projection over the real store on exact sample1', () => {
  it('reads J-03\'s fixed task in the editor\'s words without moving one stored byte', async () => {
    ...
    try {
      ...
      expect(drawer.steps).toEqual([
        { label: '准备任务输入', result: `任务输入修订版 ${prepared.checkpoint!.revisionLabel}` },
      ...
      const authorized = store.authorizeTaskAuthorization(bookId, prepared.taskIntent!.taskIntentId, prepared.planEnvelope!.digest);
  ...
  it('names the current plan version\'s range after 重新确认计划 while the Task Intent keeps the first (#288)', async () => {
    ...
    try {
      ...
      expect(firstPlan.boundary.adaptable).toEqual(['模型服务暂时出错时，同一个阅读范围安全地再试一次']);
      ...
      const authorized = store.authorizeBaselineAnalysis(bookId, first.taskIntent!.taskIntentId, first.planEnvelope!.digest);
      owner.admitAndDispatch(authorized.dispatchRunRecordId!);
      await owner.whenIdle();
      const settled = store.inspectBaselineAnalysis(bookId);
      expect(settled.state).toBe('settled');
      expect(plan(store, bookId, 'baseline-analysis').state).toEqual({ key: 'settled', label: '已完成' });
      ...
      expect(drifted.drift?.resolution).toContain('重新确认计划');

      const reconfirmed = prepare(bookId, { mode: 'reanalyze-range', selectedRange: rangeB }, true);
  ...
  it('reads a Review Run\'s plan per category and reports the newer Run that supersedes it', async () => {
    ...
    try {
      ...
      expect(drawer.steps).toEqual([
        { label: `逐章审读：${TYPOS_AND_USAGE.label}`, result: `${TYPOS_AND_USAGE.label}的发现（稿件上的标记）` },
        { label: '读取基线分析的线索：情节逻辑与前后一致', result: '情节逻辑与前后一致的发现（稿件上的标记）' },
      ...
      prepareRun(bookId, [TYPOS_AND_USAGE.categoryId]);
