...
import { BOOK_PEOPLE_SCHEMA_VERSION, EVALUATION_RECORD_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
...
describe('知识库 › 审阅规范文件 over the real store', () => {
  ...
  it('adds revision 45 to a revision-44 store with nothing else moved, and refuses a version rewritten by hand', async () => {
    ...
    try {
      plant.exec(`DROP TABLE evaluation_record_entries; DROP TABLE evaluation_records; DROP TABLE library_material_decisions; DROP TABLE library_materials; DROP TABLE review_guideline_versions; PRAGMA user_version = ${BOOK_PEOPLE_SCHEMA_VERSION};`);
    } finally {
      plant.close();
    }
