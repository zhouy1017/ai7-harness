...
import { ANALYSIS_FEEDBACK_SCHEMA_SQL, initializeAnalysisFeedbackSchema } from '../../src/service/analysis-feedback.js';
...
import { BOOK_PEOPLE_SCHEMA_SQL, initializeBookPeopleSchema } from '../../src/service/book-people.js';
...
import { initializeProductionDocumentOriginSchema, PRODUCTION_DOCUMENT_ORIGIN_SCHEMA_SQL } from '../../src/service/production-document-origins.js';
import {
...
const REVISIONS: ReadonlyArray<Revision> = [
  {
    revision: 58,
    step: initializeDatabaseMergeSchema,
    undo: (database) => drop(database, Object.keys(DATABASE_MERGE_SCHEMA_SQL).reverse()),
  },
  {
    revision: 57,
    step: initializeDatabaseReplacementSchema,
    undo: (database) => drop(database, Object.keys(DATABASE_REPLACEMENT_SCHEMA_SQL).reverse()),
  },
  {
    revision: 56,
  ...
  },
  {
    ...
    undo: (database) => drop(database, Object.keys(DATABASE_EXPORT_SCHEMA_SQL).reverse()),
  },
  {
    revision: 54,
    step: initializeDataVersionSchema,
    undo: (database) => drop(database, Object.keys(DATA_VERSION_SCHEMA_SQL).reverse()),
  },
  {
    revision: 53,
    step: initializeSeriesKnowledgeSchema,
    undo: (database) => drop(database, Object.keys(SERIES_KNOWLEDGE_SCHEMA_SQL).reverse()),
  },
  {
    revision: 52,
    step: initializeSeriesSchema,
    undo: (database) => drop(database, Object.keys(SERIES_SCHEMA_SQL).reverse()),
  },
  {
    revision: 51,
    step: initializeEvaluationCalibrationSchema,
    undo: (database) => drop(database, Object.keys(EVALUATION_CALIBRATION_SCHEMA_SQL).reverse()),
  },
  {
    revision: 50,
    step: initializeLearningEligibilitySchema,
    undo: (database) => drop(database, Object.keys(LEARNING_ELIGIBILITY_SCHEMA_SQL).reverse()),
  },
  {
    revision: 49,
    step: initializeDecisionFeedbackSchema,
    undo: (database) => drop(database, Object.keys(DECISION_FEEDBACK_SCHEMA_SQL).reverse()),
  },
  {
    revision: 48,
    step: initializeAnalysisFeedbackSchema,
    undo: (database) => drop(database, Object.keys(ANALYSIS_FEEDBACK_SCHEMA_SQL).reverse()),
  },
  {
    revision: 47,
    step: initializeEvaluationRecordSchema,
    undo: (database) => drop(database, Object.keys(EVALUATION_RECORD_SCHEMA_SQL).reverse()),
  },
  {
    revision: 46,
    step: initializeLibraryMaterialSchema,
    undo: (database) => drop(database, Object.keys(LIBRARY_MATERIAL_SCHEMA_SQL).reverse()),
  },
  {
    revision: 45,
    step: initializeReviewGuidelineSchema,
    undo: (database) => drop(database, Object.keys(REVIEW_GUIDELINE_SCHEMA_SQL).reverse()),
  },
  {
    revision: 44,
    step: initializeBookPeopleSchema,
    undo: (database) => drop(database, Object.keys(BOOK_PEOPLE_SCHEMA_SQL).reverse()),
  },
  {
    revision: 43,
  ...
  {
    revision: 41,
    step: initializeBookDeliveryPackageExportSchema,
    undo: (database) => drop(database, Object.keys(BOOK_DELIVERY_PACKAGE_EXPORT_SCHEMA_SQL).reverse()),
  },
  {
    revision: 40,
...
describe('an upgrade interrupted before its version stamp', () => {
  for (const { revision, step } of REVISIONS) {
    ...
    it(`is finished by the next open when revision ${revision}'s relations committed and its stamp did not`, async () => {
      expect(await opened()).toBe(DATABASE_MERGE_SCHEMA_VERSION);
