...
import { EDITORIAL_MARK_SCHEMA_VERSION, CLARIFICATION_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
  ...
  it('widens a revision-22 store\'s marks relation with every row as it was, and a deletion then applies and reverses on it', async () => {
    ...
    try {
      ...
      downgrade.exec(`BEGIN IMMEDIATE;
        ${[...CLARIFICATION_RELATIONS_DROP_ORDER, ...RUN_CHECKPOINT_RELATIONS_DROP_ORDER, ...DEFAULT_EXECUTION_RULE_RELATIONS_DROP_ORDER, ...EXPORT_LEDGER_RELATIONS_DROP_ORDER, ...IMPORTED_MARK_RELATIONS_DROP_ORDER, ...IMPORT_RETENTION_RELATIONS_DROP_ORDER, ...PROPOSAL_CONFLICT_RELATIONS_DROP_ORDER, ...PUBLICATION_VERSION_RELATIONS_DROP_ORDER, ...REVIEW_RUN_RELATIONS_DROP_ORDER, ...EFFECT_RELATIONS].map((relation) => `DROP TABLE ${relation};`).join('\n')}
    ...
    try {
      ...
      expect((after.prepare('PRAGMA user_version').get() as { user_version: number }).user_version).toBe(CLARIFICATION_SCHEMA_VERSION);
