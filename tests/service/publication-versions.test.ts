...
import { EDITORIAL_REVIEW_SCHEMA_VERSION, PUBLICATION_VERSION_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
import {
...
describe('⑥ 发稿: Milestone Versions and 设为发稿版本', () => {
  ...
  it('migrates a revision-24 store forward, adding the three empty relations and reading the milestones it held', async () => {
    ...
    // Plant revision 24: its relations are exactly the current ones less the three revision 25 adds.
    ...
    withDatabase(true, (database) => {
      ...
      // Exactly the three relations revision 25 adds appear, each empty; no relation the revision-24 store
      // held changed shape, and the only content that moved is the service lifetime every open appends.
      expect([...truthAfter.keys()]).toEqual([...truthBefore.keys(), ...PUBLICATION_VERSION_RELATIONS_DROP_ORDER].sort());
      for (const relation of PUBLICATION_VERSION_RELATIONS_DROP_ORDER) expect(truthAfter.get(relation)?.content).toMatch(/^0:/);
