...
import { EDITORIAL_REVIEW_SCHEMA_VERSION, CLARIFICATION_SCHEMA_VERSION, EVALUATION_CALIBRATION_SCHEMA_VERSION } from '../../src/service/task-authorization.js';
import {
  ...
  it('records the designation, its permission and both events in one transaction, and a failure midway records none of them', async () => {
    ...
    try {
      ...
      expect(deliverables.publication.actualsPrompt).toEqual({
        ...
        // Nothing entered yet in 设置 › 评估校准与预测 (Issue #430, S82).
        actuals: null,
      });
