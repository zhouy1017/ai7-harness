...
- evaluate observable behavior with deterministic mock-provider cases, journey tests, and separately authorized live-provider evidence;
...
It does not mean that the production agent may silently rewrite its own plugins, prompts, policies, or authority. A future design may let agents propose behavior changes, but activation authority, validation, rollout, and rollback remain open under Question 29.
...
| Replay and snapshots | `docs/testing.md`; `packages/test-support/llm-replay/`; `packages/test-support/acp-snapshot/`; `vitest.snapshot.config.ts`; `vitest.web.config.ts` | Keyless regression evidence for effective prompts, tool schemas, model chunks, persisted logs, protocol output, and UI | Replay can be nondeterministic for concurrent sibling subagents. Snapshots prove deterministic behavior, not correctness. |
...
Therefore the initial AI7 production composition uses a pinned bundle, committed system presets, static plugins, scoped prompt/context providers, static tools, explicit policy layers, durable session events, and AI7-owned evaluation. Dynamic authoring and self-referential Cordis stay behind developer/experimental trust gates unless Question 29 accepts a narrower governed use.
...
## Proposed behavior-improvement lifecycle

1. Pin a Foundation Model requirement and a Harness Behavior Composition.
2. Run provider-free generated cases and representative editorial journeys; use live-provider evaluation only when separately authorized.
