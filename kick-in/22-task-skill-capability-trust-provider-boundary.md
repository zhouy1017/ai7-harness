...
| Credentials | Ordinary configuration accepts only opaque references; the predecessor Windows adapter uses Credential Manager. | Keep references-only and require an OS-protected backend on every supported platform; deepen per-consumer brokering. |
...
| Opaque credential references and OS protection | Keep/deepen | AI7 Credential Broker plus a supported-platform protected backend and per-consumer ACL. |
