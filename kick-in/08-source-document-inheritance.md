...
- Recommend that identical and non-interacting different-block changes may auto-merge, while different same-block changes and structural interactions require explicit editor resolution; model-generated combined text may be a proposal only. A Proposal that targets an exact range inside a block conflicts by that range ([ADR 0085](../docs/adr/0085-decide-a-ranged-proposal-s-conflict-by-the-words-it-replaces.md)).
