...
1. **Source Version** — immutable imported evidence that may seed editable text; it remains part of textual-source lineage.
...
6. **Manuscript Checkpoint** — the transition that validates complete journal-reconstructed state and commits it as a Manuscript Revision.
...
1. Branches version editable manuscript text, never the complete Book project or its approvals, lifecycle, source records, or production state.
...
5. Dirty working state receives a labeled safety checkpoint before branch switching, merge, restore, reimport reconciliation, bulk apply, or other graph-changing work. Checkpoint failure aborts without discarding journaled edits.
...
7. The default versioned merge policy auto-merges only identical changes and non-interacting changes to different stable blocks. Different edits within the same block and ambiguous structural interactions become conflicts even if character ranges appear disjoint. A Proposal that targets an exact range inside a block — a 修改建议 — is the exception: it conflicts only when a change touches the words it replaces, and a change elsewhere in its block is a Safe Non-interacting Merge ([ADR 0085](../docs/adr/0085-decide-a-ranged-proposal-s-conflict-by-the-words-it-replaces.md)). AI7 may propose a combined resolution, but an editor decides it.
...
- General DOCX/source reimport needs a host-neutral structural reconciliation contract; the current strongest implementation is Word-paragraph-specific.
- Initial block seeding is deterministic inside one import, but continuity across a newly imported Source Version is not automatically established.
...
Question 17 accepted the semantic model and keep/modify/drop boundary above, including the conservative default that different concurrent edits to the same stable block require explicit editor resolution. The choice does not adopt any legacy storage or UI implementation. See [ADR 0006](../docs/adr/0006-preserve-manuscript-native-history-and-recovery.md).
