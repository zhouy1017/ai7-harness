...
Pull requests remain Draft during authoring, review, rebase, and local validation. The Commander makes an integration-ready pull request Ready, which normally arms the one hosted occurrence. Newer pushes while Ready cancel a superseded in-progress same-PR occurrence and start the newest occurrence. A product/bootstrap/build/journey failure returns the pull request to Draft; only the Commander may rerun one clearly external runner/network/infrastructure transient without a code change. This lifecycle bounds usage and does not create an exact-head, same-SHA, formal-review, or zero-finding gate.

During an exact Owner suspension of hosted Actions, Workers continue the unchanged local completion sequence and Commanders may push branches or maintain Draft pull requests. Product changes normally remain unmerged. Only while [ADR 0050](../docs/adr/0050-waive-hosted-e2e-integration-evidence-during-actions-exhaustion.md) is repository-current and its exact disabled-workflow, no-run, and no-confirmed-reset conditions hold may the Commander make an otherwise-ready product pull request Ready and integrate it without hosted evidence. Immediately before Ready and again before merge, the Commander records those conditions and includes ADR 0050's exact disclosure. Waived product pull requests integrate one at a time in authorized dependency order; no local result, advisory review, fake green, substitute Gate, or single-platform run is claimed as hosted Gate evidence. Pure documentation, design, and CI-governance work use only the path recorded in the current [CI boundary](../docs/agents/ci-test-boundaries.md). Independent branches consume stable owners/interfaces on current `dev`; any necessary stacked dependency and integration order must be explicit and separately authorized.

Authoritative confirmation of a fresh usable Actions allocation after reset expires the waiver immediately. A Ready but unmerged product pull request returns to the normal paired-platform Gate lifecycle. Only after that confirmation, the Commander may use ADR 0050's prospective Owner authority to re-enable exact workflow `342459594` as a separate explicit action and resume ADR 0049. No probe, automatic enablement, manual dispatch, or retrospective run is authorized.
