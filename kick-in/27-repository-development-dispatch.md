...
A Reviewer's Return Receipt repeats `reviewed_head` and its `class_match`: on `codex` whether its binding matched the reviewed class, on `claude-code` the literal `fixed-reviewer-binding` plus the reviewed class, because the Reviewer binding is fixed by route under ADR 0068. Receipts never claim effective runtime-model attestation.

## Parallel work and integration

- One Issue owns one branch, one pull request, and one writable Worker.
- At most three active Worker Task Sessions across all harnesses and modes, only when their slices consume stable owners on current `dev`; never split work to fill slots. Electron Journeys run on the background test guests of [ADR 0083](../docs/adr/0083-run-every-agent-s-electron-journeys-on-the-background-test-guests.md), one run per guest at a time; when no guest is idle the Commander tells a Worker to hold its Journey rung.
