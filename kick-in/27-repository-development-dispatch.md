...
During an exact external CI condition, Workers continue local development and Commanders may push branches or maintain Draft pull requests. A product change integrates without hosted evidence only while [ADR 0053](../docs/adr/0053-preserve-local-first-development-through-a-bounded-ci-degraded-mode.md) and the CI boundary's exact activation rules hold. Immediately before Ready and again before merge, the Commander records those conditions, enforces the affected-platform stop, obtains fresh all-four-Journey Local completion at the rebased head, and includes the exact disclosure. Product pull requests integrate one at a time from current `dev`; no local result, advisory review, fake green, substitute Gate, or single-platform run is claimed as hosted Gate evidence. Pure documentation, design, and CI-governance work uses only its Change Brief's applicable local validation. Independent branches consume stable owners/interfaces on current `dev`; any necessary stacked dependency and integration order must be explicit and separately authorized.

Authoritative resolution of the external condition immediately returns a Ready but unmerged product pull request to the normal paired-platform Gate lifecycle. Workflow enablement or dispatch requires separate exact external-action authority. No probe, automatic enablement, manual dispatch, replacement workflow, or retrospective run is authorized.
