...
| Worktree | `.claude/worktrees/<generated-name>` on a generated branch, both created by the harness; the start commit follows the harness's own worktree base setting, never the Agent call, so step 4 of the two-stage launch enforces the exact start; the harness removes a worktree that is still unchanged when the agent ends, which is the state a preflight turn that writes nothing always leaves behind, so that turn instead leaves exactly one untracked, uncommitted scratch marker `.ai7-preflight-<dispatch_id>` at the worktree root—the only write the preflight may perform—to keep the worktree alive for the attach step |
...
| Attach step for a Worker | Reset a clean checkout to the exact start when needed, then rename the generated branch to the reserved Issue branch, operating on the worktree path from the Commander session while the subagent is idle; the step depends on the worktree that the preflight marker kept alive, and that marker is untracked scratch rather than a controlled file, so a checkout whose only `git status --porcelain` entry is `?? .ai7-preflight-<dispatch_id>` is still clean for this step and survives the reset |
...
2. The Commander launches a fresh Task Session—`create_thread` for a Codex top-level session; a hand-created desktop session with worktree isolation, a short worktree name, and the exact requested model and effort for a Claude Code top-level session; one Agent tool call with `run_in_background: true`, `isolation: "worktree"`, the explicit class model alias, and the general-purpose subagent type for a `subagent`; or the target harness's CLI in the background with the exact model and effort for a `cli-session`—with a first prompt containing only the Issue URL, dispatch ID, expected Brief revision/body hash, Commander harness, attempt harness, launch mode, role/class/binding, named branch, exact base/target, the `reviewed_head` when the role is Reviewer, and a no-write bootstrap-preflight instruction. The prompt requires the session to report its actual cwd/worktree and current branch-or-detached state. In the `subagent` mode that instruction also names the retention marker, because a turn that writes nothing ends with an unchanged worktree the harness removes: the prompt directs the session to leave exactly one untracked, uncommitted scratch marker `.ai7-preflight-<dispatch_id>` at the worktree root as the preflight's only write, never to stage or commit it, and to delete it itself once it has verified the Launch Receipt.
3. The Worker or Reviewer reads root `AGENTS.md`, the Issue, and those immutable launch expectations, then verifies the hash from a byte-preserving API read, the actual checkout at the role-specific start commit, and every start/stop condition read-only. It reports `preflight-ready` or `needs-commander` and stops without expecting a Receipt or editing controlled files. In the `subagent` mode it also creates the retention marker before its turn ends, so the worktree the attach step needs still exists; that marker is the single exception to the no-write rule, and it is untracked scratch, never a controlled file. In the `subagent` mode that report is the subagent's turn result, delivered to the Commander by the completion notification; in the `cli-session` mode it is the CLI's final message for that run.
4. After that first turn stops, the Commander verifies the launch-accepted binding through the mode's evidence—desktop metadata or the accepted `create_thread` request for a top-level session, the Agent call parameters it sent for a subagent, the CLI arguments it sent plus any start event for a `cli-session`—the generated worktree, confirmed against `git worktree list` and the preflight report, and a clean checkout at the role-specific start. In the `subagent` mode that worktree exists only because the preflight left its retention marker; clean there means a clean tracked tree whose sole untracked entry is that marker. For a Worker it then applies the attach step from the route tables, only while the checkout is clean. A Reviewer stays detached and read-only at `reviewed_head`; never attach it to a branch. The Commander sets the session title where the mode has one (fixed at creation by `description` in the `subagent` mode; `none` in the `cli-session` mode), posts the finalized Launch Receipt as one GitHub Issue comment, and sends only that immutable comment permalink to the session through `send_message_to_thread`, `send_message`, `SendMessage` to the `agent_id`, or the CLI's resume command.
5. The Worker or Reviewer fetches the exact comment, verifies every field against the Issue, session, checkout and preflight facts, and only then begins authorized work. In the `subagent` mode it deletes its own retention marker immediately after that verification and before its first controlled-file edit; the marker is the Worker's to remove, never the Commander's, and it must never appear in any commit, `git add`, or pull request. A mismatch returns `needs-commander` without a controlled-file edit, leaving the marker in place so the worktree survives for the Commander's decision.

There is no mutable `pending` receipt and no claim that session creation plus receipt publication is atomic. No launcher script, daemon, or connector is part of this sequence; the Agent tool and the target harness's CLI are invoked directly by the Commander. If the harness has already removed an unchanged worktree before step 4 completes—the failure the `subagent` retention marker exists to prevent—the Commander closes the attempt `failed` with its Return Receipt and launches a fresh attempt.
...
- for a `subagent`, launch evidence that names the retention marker `.ai7-preflight-<dispatch_id>` the preflight left at the worktree root, records that it kept the worktree alive for the attach step, and states that it is untracked scratch the Worker deletes itself and that appears in no commit;
