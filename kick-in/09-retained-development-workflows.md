...
The source had not completed its required real Host observations at the pin. Current treatment under ADR 0060 is therefore:
...
- Use fresh top-level Claude Code Task Sessions plus Claude desktop session queries instead of copying the large repository-specific state machine.
...
- Question 25 is closed. The current contract is in [Repository Development Dispatch](./27-repository-development-dispatch.md) and [ADR 0060](../docs/adr/0060-dispatch-repository-work-through-issue-bound-claude-code-sessions.md); [ADR 0059](../docs/adr/0059-dispatch-repository-work-through-issue-bound-codex-task-sessions.md) and [ADR 0015](../docs/adr/0015-provider-neutral-development-dispatch.md) are superseded history. Read the pilot inventory above through ADR 0060: exact-start isolation (Worker base; Reviewer `reviewed_head`), one writer, receipts, and Commander authority survive, while the provider-neutral/Claude/Spark/fallback design, Codex-only tooling, lifecycle state machine, `agent-host-connector/`, DPAPI, and Windows Hello enrollment remain historical rather than reimplemented.
