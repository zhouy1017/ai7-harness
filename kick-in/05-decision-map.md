...
| 25 | Local development multi-agent dispatch contract | Three roles — Commander, Worker, independent Reviewer — with fixed class-specific Claude Code bindings; every Worker/Reviewer attempt starts in a fresh Issue-bound Task Session behind a verified two-stage Launch Receipt; query Claude desktop session state plus Issue receipts, parallelize at most three independent Workers, and keep integration/external actions with Commander | **Accepted; replaced by ADR 0059 on 2026-09-03 and rebound by ADR 0060 on 2026-09-04** | Repository-agent runbook |
...
**Question 25 is closed.** Current Repository Development Dispatch uses fixed Claude Code bindings and one fresh Issue-bound Task Session per Worker/Reviewer attempt, with two-stage Launch/Return Receipts and a query-only Dispatch Register. It is recorded in [27-repository-development-dispatch.md](./27-repository-development-dispatch.md) and [ADR 0060](../docs/adr/0060-dispatch-repository-work-through-issue-bound-claude-code-sessions.md). [ADR 0059](../docs/adr/0059-dispatch-repository-work-through-issue-bound-codex-task-sessions.md) preserves the superseded Codex-only tooling and [ADR 0015](../docs/adr/0015-provider-neutral-development-dispatch.md) the superseded provider-neutral/Claude/Spark/fallback design as history; the legacy orchestration pilot and host connector also remain evidence only.
