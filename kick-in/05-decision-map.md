...
| 25 | Local development multi-agent dispatch contract | Three roles — Commander, Worker, independent Reviewer — with fixed class-specific bindings per harness (Codex or Claude Code) selected per attempt by the Commander; every Worker/Reviewer attempt starts in a fresh Issue-bound Task Session—a top-level session, a Claude Code Agent-tool subagent, or a CLI-launched session, the only mode across harnesses—behind a verified two-stage Launch Receipt; query that harness's session state plus Issue receipts, parallelize at most three independent Workers, and keep integration/external actions with Commander | **Accepted; replaced by ADR 0059 on 2026-09-03, rebound by ADR 0060 and harness-routed by ADR 0061 on 2026-09-04, then extended the same day by ADR 0061's subagent launch mode and ADR 0063's cross-harness CLI launch** | Repository-agent runbook |
...
**Question 25 is closed.** Current Repository Development Dispatch uses fixed per-harness bindings selected per attempt by the Commander (Codex or Claude Code) and one fresh Issue-bound Task Session per Worker/Reviewer attempt—launched as a top-level session, a Claude Code Agent-tool subagent, or a CLI-launched session, the only mode for a cross-harness attempt—with two-stage Launch/Return Receipts and a query-only Dispatch Register. It is recorded in [27-repository-development-dispatch.md](./27-repository-development-dispatch.md), [ADR 0061](../docs/adr/0061-route-repository-dispatch-by-commander-harness.md), and [ADR 0063](../docs/adr/0063-allow-cross-harness-dispatch-through-cli-launched-task-sessions.md). [ADR 0059](../docs/adr/0059-dispatch-repository-work-through-issue-bound-codex-task-sessions.md) and [ADR 0060](../docs/adr/0060-dispatch-repository-work-through-issue-bound-claude-code-sessions.md) preserve the superseded single-harness exclusivity, and [ADR 0015](../docs/adr/0015-provider-neutral-development-dispatch.md) the superseded provider-neutral/Claude/Spark/fallback design, as history; the legacy orchestration pilot and host connector also remain evidence only.
