...
**Question 25 is the current question**: the Repository Development Dispatch contract — roles, authority ceilings, worktree ownership, handoffs, receipts, and the relationship to available development-agent tooling. Its direction was accepted at Question 6 (keep local multi-agent dispatch, development-only, never product runtime); only the detail is open. The pinned source inventory is in [09-retained-development-workflows.md](./09-retained-development-workflows.md).
