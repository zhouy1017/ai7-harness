...
| 24 | Exact verification tiers and generated mock-provider corpus | Superseded. The four-tier proposal was answered with an owner correction, not an acceptance: the target platform is Windows-only, Ubuntu has no production role, and the tiered verification/build/test must be concise and quick. A revised proposal is required | **Asked; answered with correction; revised proposal owed** | Testing strategy |
| 25 | Local development multi-agent dispatch contract | Keep/revise the direction already accepted; decide roles, authority, handoffs, and receipts outside product runtime | **Direction accepted; detail open** | Repository-agent runbook |
...
**Question 24 is the current question, and it is open.** It was asked with a four-tier proposal and answered with a correction rather than an acceptance:
...
Only the Question 6 preservation direction — a tiered GitHub Actions workflow combined with generated mock-LLM-provider cases, required CI provider-free — remains accepted. The detailed contract in [26-tiered-verification-and-mock-provider-evidence.md](./26-tiered-verification-and-mock-provider-evidence.md) is superseded and must be re-proposed under the Windows-only and concise/quick constraints before any part of it becomes project truth.
