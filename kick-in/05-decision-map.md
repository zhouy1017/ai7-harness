...
| 25 | Local development multi-agent dispatch contract | Three roles — Commander, Worker, independent Reviewer — with fixed class-specific bindings per harness (Codex or Claude Code) routed by the Commander's harness; every Worker/Reviewer attempt starts in a fresh Issue-bound Task Session behind a verified two-stage Launch Receipt; query that harness's session state plus Issue receipts, parallelize at most three independent Workers, and keep integration/external actions with Commander | **Accepted; replaced by ADR 0059 on 2026-09-03, rebound by ADR 0060 and harness-routed by ADR 0061 on 2026-09-04** | Repository-agent runbook |
